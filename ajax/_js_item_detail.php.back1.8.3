...
<?if($arPost["PARAMS"]):?>
	...
	if($arItems)
	{
		...
		foreach($arItems["ITEMS"] as $key => $arItem)
		{
			...
			$arItems["ITEMS"][$key]["AVAILIABLE"] = CMax::GetQuantityArray($arItem["TOTAL_COUNT"], array(), isset($arPost["PARAMS"]['USE_STORE_CLICK']) ? $arPost["PARAMS"]['USE_STORE_CLICK'] : 'N');
			...
			$arItems["ITEMS"][$key]["ONE_CLICK_BUY_HTML"] = \Aspro\Functions\CAsproMax::showItemOCB($arAddToBasketData, $arItem, $arPost["PARAMS"], true, $ocbButtonClass);
