...
class CBitrixLocationSelectorSearchComponent_new extends CBitrixComponent
{
	...
	public static function processSearchRequestV2($parameters)
	{
        ...
        $name =  $parameters['filter']['=PHRASE'];
        $name = strtoupper($name);

        $arQuery = [
            'if( %1$s LIKE ' . "'%%{$name}%%', 60, 0)"
        ];
        $arName = explode(' ', $name);
        foreach ($arName as $word) {
            $word = strtoupper($word);
            $arQuery[] = 'if( %1$s LIKE ' . "'%%{$word}%%', 10, 0)";
        }
...
//                ['=SITE_ID'] => 's2'
            ...
            'additionals' => ['PATH'],
            'runtime' => array(
                new Main\Entity\ExpressionField('SCORE', "({$query})", 'NAME.NAME_UPPER'),
            ),
            'order' => [
                'SCORE' => 'DESC',
                'TYPE.SORT' => 'ASC',
