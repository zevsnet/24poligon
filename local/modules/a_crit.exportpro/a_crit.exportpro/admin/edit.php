...
<?}
    ...
    if( $ID > 0 ){
        ...
        $aMenu[] = array(
            ...
            "LINK" => "/local/tools/a_crit.exportpro/acrit_exportpro.php?ID=".$ID,
						"PUBLIC" => "Y",
    ...
    $bUnlockMode = file_exists( $_SERVER["DOCUMENT_ROOT"]."/local/tools/".$moduleId."/export_{$ID}_run.unlock" );
    ...
    if( !isset( $_REQUEST["ajax"] ) && !isset( $_REQUEST["ib"] ) && !isset( $_REQUEST["ajax_start"] ) && !isset( $_REQUEST["ajax_count"] ) && !isset( $_POST["auth"] ) ){
        ...
        <?// check session id?>
            ...
            $tabControl->Buttons(
                array(
                    "disabled" => ( ( $POST_RIGHT < "W" ) || file_exists( $_SERVER["DOCUMENT_ROOT"]."/local/tools/".$moduleId."/export_{$arProfile["ID"]}_run.unlock" ) ),
