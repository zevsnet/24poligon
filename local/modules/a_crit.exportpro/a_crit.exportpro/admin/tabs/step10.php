...
if( ( $arProfile["TYPE"] == "activizm" ) && !empty( $categories ) ){
    if( $options && is_array( $options ) && !empty( $options ) ) {
        $propertyList = $options;
    }
    else{
        $propertyList = $obProfileUtils->createFieldset2( $arProfile["IBLOCK_ID"], true );
    }

    $catalogPrice = array( "CATALOG" => $propertyList["CATALOG"] );
    unset( $catalogPrice["CATALOG"]["QUANTITY"] );
    $propertyList = $obProfileUtils->selectFieldset2( $propertyList, "" );
    $catalogPrice = $obProfileUtils->selectFieldset2( $catalogPrice, $arProfile["VARIANT"]["PRICE"] );
...
<?if( ( $arProfile["TYPE"] == "activizm" ) && !empty( $categories ) ){?>
    ...
    <tr align="center">
        <td colspan="2">
            <?=BeginNote();?>
            <?=GetMessage( "ACRIT_EXPORTPRO_VARIANT_LIST_SEX_DESCRIPTION" );?>
            <?=EndNote();?>
        </td>
    </tr>
    <tr>
    ...
    <tr align="center">
        <td colspan="2" style="width: 200%; height: 205px; display: inline-block;">
            <?=BeginNote();?>
            <?=GetMessage( "ACRIT_EXPORTPRO_VARIANT_LIST_COLOR_DESCRIPTION" );?>
            <?=EndNote();?>
        </td>
    </tr>
    <tr>
    ...
    <?foreach( $categories as $id => $category ){?>
        <tr class="variant">
            <td>
                <label for="PROFILE[VARIANT][CATEGORY_EXT][<?=$category["ID"]?>]">
                    <?=$category["NAME"]?>
                </label>
            </td>
            <td>
                <?$variantCategory = CExportproVariant::GetCategorySelect(
                    array(
