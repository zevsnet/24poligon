...
class CAcritExportproExport{
    ...
    public function __construct( $profileID ){
        global $exportstep;
        $this->lockDir = $_SERVER["DOCUMENT_ROOT"]."/local/tools/a_crit.exportpro/";

        $this->dbMan = new CExportproProfileDB();
        $this->profile = $this->dbMan->GetByID( $profileID );

        $siteDocumentRoot = CAcritExportproTools::GetSiteDocumentRoot( $this->profile["LID"] );
        $this->documentRoot = ( $siteDocumentRoot ) ? $siteDocumentRoot : $_SERVER["DOCUMENT_ROOT"];

        $this->baseDir = $this->documentRoot."/a_crit.exportpro/";
        ...
        if( strlen( strstr( self::$fileExport, $this->baseDir ) ) > 0 ){
            self::$fileExport = str_replace( $this->baseDir, $this->documentRoot."/upload/a_crit.exportpro/", self::$fileExport );
    ...
    public function Export( $type = "", $cronpage = 0, $processId = 0 ){
        ...
        if( file_exists( $_SERVER["DOCUMENT_ROOT"]."/local/tools/a_crit.exportpro/export_{$this->profile["ID"]}_run.unlock" )
            || ( $bBaseCron && file_exists( $_SERVER["DOCUMENT_ROOT"]."/local/tools/a_crit.exportpro/export_{$this->profile["ID"]}_run.lock" ) && !$cronpage ) ){
            if( !$bBaseCron ){
                $this->Unlock();
            }
            return false;
		...
		$obLog = new CAcritExportproLog( $this->profile["ID"] );
        ...
        if( CModule::IncludeModule( "catalog" ) ){
            ...
            CAcritExportproProps::$arIBlockFilter = $obProfileUtils->PrepareIBlock( $this->profile["IBLOCK_ID"], $this->profile["USE_SKU"] );
        ...
        if( !$end ){
                    ...
                    for( $i = 0; $i < $steps2; $i++ ){
                        for( $j = 0; $j < $tCnt; $j++ ){
                            ...
                            $threadsId = $threads->newThread(
                                $this->documentRoot."/local/modules/a_crit.exportpro/tools/cronrun_proc.php",
                                array(
            ...
            if( !$allPages && $allPages != -1 && !$cronrun ){
                ...
                if( ( $this->profile["SETUP"]["FILE_TYPE"] != "csv" ) && ( $this->profile["SETUP"]["FILE_TYPE"] != "xls" ) ){
                    ...
                    $defaultFields = array(
                        ...
                        "#CATEGORY#" => ( ( $this->profile["USE_MARKET_CATEGORY"] == "Y" ) && ( $this->profile["CHANGE_MARKET_CATEGORY"] == "Y" ) && ( $this->profile["TYPE"] != "google" ) ) ? $this->GetYandexCategoryXML( CAcritExportproTools::GetSections( $this->profile ) ) : $this->GetCategoryXML( CAcritExportproTools::GetSections( $this->profile ) ),
                        "#CURRENCY#" => ( CModule::IncludeModule( "catalog" ) ) ? $this->GetCurrencyXML( CAcritExportproTools::GetCurrencies( $this->profile ) ) : "RUB",
                ...
                if( CAcritExportproTools::HasBadCronExports( $this->profile ) ){
                    $arSupportData[] = GetMessage( "ACRIT_EXPORTPRO_PROCESS_CRON_BAD_EXPORTS" );
                }
                ...
                if( $this->profile["SETUP"]["FILE_TYPE"] != "xls" ){
                    self::SaveToOut();
                }
                ...
                if( $this->profile["USE_COMPRESS"] == "Y" ){
                    ...
                    elseif( file_exists( $this->documentRoot."/upload/a_crit.exportpro/".$this->originalNamePath ) ){
                        $zipSavePath = $this->documentRoot."/upload/a_crit.exportpro/".$this->originalNamePath;
                    }
                    ...
                    $zipPath = ( stripos( $zipSavePath, ".xml" ) !== false ) ? str_replace( ".xml", ".zip", $zipSavePath ) : ( ( stripos( $zipSavePath, ".csv" ) !== false ) ? str_replace( ".csv", ".zip", $zipSavePath ) : str_replace( ".xls", ".zip", $zipSavePath ) );
    ...
    private function Unlock(){
        ...
        AcritExportproSession::DeleteSession( $this->profile["ID"], $_SERVER["DOCUMENT_ROOT"]."/local/tools/a_crit.exportpro/" );
    }
    ...
    private function GetCategoryXML( $sections ){
        ...
        foreach( $sections as $sectionId ){
            $dbSectionList = CAcritExportproTools::GetSectionTree(
                false,
                $sectionId
            );
