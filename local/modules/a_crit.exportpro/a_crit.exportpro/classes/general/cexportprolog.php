...
class CAcritExportproLog{
    public $logFilename = "/upload/a_crit.exportpro/";
    private $profileId;
    private $session;
    ...
    public function GetLog( $profileID, $bSendEmailReport = true, $arSupportData = false ){
        ...
        if( $bSendEmailReport ){
            ...
            if( check_email( $arProfile["SEND_LOG_EMAIL"] ) ){
                ...
                $statVal = "";
                if( !$exportTimeStamp ){
                    $statVal = GetMessage( "ACRIT_EXPORTPRO_LOG_STAT_GENERATE" );
                }
                elseif( $exportTimeStamp < $profileTimeStamp ){
                    $statVal = GetMessage( "ACRIT_EXPORTPRO_LOG_STAT_REGENERATE" );
                }
                else{
                    if( $arProfile["SETUP"]["TYPE_RUN"] == "comp" ){
                        $statVal = GetMessage( "ACRIT_EXPORTPRO_LOG_STAT_FINISHED" );
                    }
                    else{
                        $maxCronProducts = 0;
                        ...
                        if( !$maxCronProducts ){
                            ...
                            if( $unloandedPercent >= 100 ){
                                ...
                                $statVal = GetMessage( "ACRIT_EXPORTPRO_LOG_STAT_IN_PROCESS_BEGIN" )." ".$arProfile["UNLOADED_OFFERS_CORRECT"]." ".GetMessage( "ACRIT_EXPORTPRO_LOG_STAT_IN_PROCESS_END" ).$unloandedPercent;
                            }
                        }
                    }
                }
                ...
                if( is_array( $arSupportData ) && !empty( $arSupportData ) ){
                    $messageBlock .= GetMessage( "ACRIT_EXPORTPRO_LOG_STAT_SUPPORT_DATA" )."\n";
                ...
                if( strlen( $statVal ) > 0 ){
                    $messageBlock .= GetMessage( "ACRIT_EXPORTPRO_LOG_STAT" ).$statVal."\n\n";
                }
                
                $messageBlock .= GetMessage( "ACRIT_LOG_PROFILE" ).$arProfile["SITE_PROTOCOL"]."://".$arProfile["DOMAIN_NAME"]."/bitrix/admin/acrit_exportpro_edit.php?ID=".$profileID."\n".            
