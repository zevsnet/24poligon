...
class AcritExportproSession{
    ...
    static public function Init( $cronpage ){
        if( !self::$sessionDir )
            self::$sessionDir = $_SERVER["DOCUMENT_ROOT"]."/local/tools/a_crit.exportpro/";
    ...
    static public function DeleteSession( $id, $sessionDirPath = false ){
        $sessionDir = ( $sessionDirPath ) ? $sessionDirPath : self::$sessionDir;

        if( file_exists( $sessionDir."export_{$id}.session" ) )
            unlink( $sessionDir."export_{$id}.session" );
