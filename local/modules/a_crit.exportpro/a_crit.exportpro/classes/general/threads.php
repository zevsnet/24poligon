...
class Threads{
    ...
    public function newThread( $filename, $params = array() ){
        ...
        if( $params["phpPath"] != "" ){
            $this->phpPath = $params["phpPath"];
        }

        $params = addcslashes( serialize( $params ), '"' );
		if(defined('BX_UTF') && BX_UTF === true) {
			$command = $this->phpPath." -d mbstring.func_overload=2 -d mbstring.internal_encoding=UTF-8 -q ".$filename.' exportparams "'.$params.'"';
		}
		else {
			$command = $this->phpPath." -d mbstring.func_overload=0 -d mbstring.internal_encoding=CP1251 -q ".$filename.' exportparams "'.$params.'"';
		}
        ++$this->lastId;
    ...
    public static function getParams(){
        foreach( $_SERVER["argv"] as $key => $argv ){
            if( $argv == "exportparams" && isset( $_SERVER["argv"][$key + 1] ) ){
