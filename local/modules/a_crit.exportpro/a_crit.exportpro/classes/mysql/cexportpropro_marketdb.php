...
class CExportproMarketDB{
    ...
    public function GetList(){
        global $DB;
        $arResult = array();
        $dbRes = $DB->Query( "SELECT * FROM {$this->tableName} ORDER BY id" );
        if( $dbRes ){
    ...
    public function GetMarketList( $marketId ){
        ...
        $dbResult = $DB->Query( "SELECT * FROM {$this->tableName} WHERE id='".$marketId."'" );
        if( $dbResult && ( $arItem = $dbResult->Fetch() ) ){
            $arMarketCategory = explode( PHP_EOL, $arItem["data"] );
            array_walk( $arMarketCategory, "CAcritExportproStringProcess::AcritArrayTrimFunc" );
        }
