...
if( !class_exists( "CExportproInformerGadget" ) ){
    class CExportproInformerGadget{
        public static $moduleId = "a_crit.exportpro";
        public static $modulePrefix = "a_crit";
...
$arUpdateData = array();
$arModuleData = CExportproInformerGadget::GetMarketModulesInfo( $arUpdateData );
    ...
    <?if( is_array( $arUpdateData ) && !empty( $arUpdateData ) ){?>
        <div class="show-info-row-version">
            ...
            <?foreach( $arUpdateData as $updateDataVersion => $updateDataDescription ){?>
                <b><?=GetMessage( "GD_ACRIT_EXPORTPRO_EXPORT_HAS_UPDATE" );?><?=$updateDataVersion?></b><br/>
            ...
            <?}?>
				...
				<div class="updates-more-info">
					<a href="/bitrix/admin/update_system_partner.php?lang=<?=LANGUAGE_ID;?>&tabControl_active_tab=tab2" target="_blank">
						<?=GetMessage( "GD_ACRIT_EXPORTPRO_MORE" );?>
					</a>
				</div>
    <?} elseif ( is_array( $arUpdateData ) && empty( $arUpdateData ) ){?>
        <div class="show-info-row-version">
						<br/>
            <?=GetMessage( "GD_ACRIT_EXPORTPRO_EXPORT_UPDATED" );?>
        </div>
		<?}?>
    <?if( $showBuyText ){?>
