...
class a_crit_exportpro extends CModule{
    const MODULE_ID = "a_crit.exportpro";
    var $MODULE_ID = "a_crit.exportpro";
    var $MODULE_VERSION;
    ...
    function InstallDB( $arParams = array() ){
        ...
        $this->errors = $DB->RunSQLBatch( $_SERVER["DOCUMENT_ROOT"]."/local/modules/".self::MODULE_ID."/install/db/install.sql" );
    ...
    function UninstallDB( $arParams = array() ){
        ...
        if( !array_key_exists( "save_tables", $arParams ) || ( $arParams["save_tables"] != "Y" ) ){
            $this->errors = $DB->RunSQLBatch( $_SERVER["DOCUMENT_ROOT"]."/local/modules/".self::MODULE_ID."/install/db/uninstall.sql" );
        }
    ...
    function DoInstall(){
        ...
        if( !isset( $step ) || ( $step < 1 ) ){
            $APPLICATION->IncludeAdminFile( GetMessage( "ACRIT_EXPORTPRO_RECOMMENDED" ), $DOCUMENT_ROOT."/local/modules/{$this->MODULE_ID}/install/step.php" );
        }
...
//                $bBitrixCloudMonitoring = CExportproInformer::UpdateBitrixCloudMonitoring( GetMessage( "ACRIT_EXPORTPRO_BITRIX_CLOUD_MONITORING_EMAIL" ) );
            ...
            $APPLICATION->IncludeAdminFile( GetMessage( "MOD_INST_OK" ), $DOCUMENT_ROOT."/local/modules/{$this->MODULE_ID}/install/step3.php" );
        }
        elseif( $step == 2 ){
            CheckDirPath( __DIR__."/db/category" );
            CopyDirFiles( __DIR__."/db/", __DIR__."/db/category" );
            $APPLICATION->IncludeAdminFile( GetMessage( "MOD_INST_OK" ), $DOCUMENT_ROOT."/local/modules/{$this->MODULE_ID}/install/step2.php" );
        }
    }
    ...
    function DoUninstall(){
        ...
        if( $_REQUEST["step"] < 2 ){
            ...
            $APPLICATION->IncludeAdminFile( GetMessage( "MOD_UNINST_OK" ), $DOCUMENT_ROOT."/local/modules/{$this->MODULE_ID}/install/unstep.php" );
        }
        elseif( $_REQUEST["step"] == 2 ){
            ...
            $DB->Query( "DELETE FROM b_option WHERE `MODULE_ID`='{$this->MODULE_ID}' AND `NAME`='~bsm_stop_date'" );
            $APPLICATION->IncludeAdminFile( GetMessage( "MOD_UNINST_OK" ), $DOCUMENT_ROOT."/local/modules/{$this->MODULE_ID}/install/unstep2.php" );
        }
    }
    ...
    function InstallFiles( $arParams = array() ){
        ...
        if( $_ENV["COMPUTERNAME"] != "BX" ){
            ...
            CopyDirFiles( $_SERVER["DOCUMENT_ROOT"]."/local/modules/".$this->MODULE_ID."/install/gadgets", $_SERVER["DOCUMENT_ROOT"]."/bitrix/gadgets", true, true );
            CopyDirFiles( $_SERVER["DOCUMENT_ROOT"]."/local/modules/".$this->MODULE_ID."/install/tools", $_SERVER["DOCUMENT_ROOT"]."/local/tools", true, true );
            foreach( $this->siteArray as $sireDir ){
                CopyDirFiles( $_SERVER["DOCUMENT_ROOT"]."/local/modules/".$this->MODULE_ID."/install/public", $sireDir, true, true );
            }
        }
        return true;
    }
    ...
    function UnInstallFiles(){
        ...
        if( $_ENV["COMPUTERNAME"] != "BX" ){
            DeleteDirFilesEx( "/bitrix/js/a_crit.exportpro/" );
            DeleteDirFilesEx( "/bitrix/gadgets/a_crit/exportpro/" );
            DeleteDirFilesEx( "/local/tools/a_crit.exportpro/" );
            DeleteDirFilesEx( "/bitrix/themes/.default/a_crit/exportpro/" );
            DeleteDirFilesEx( "/bitrix/themes/.default/a_crit/exportpro/" );
            DeleteDirFilesEx( "/bitrix/themes/.default/a_crit.exportpro.css" );
            DeleteDirFilesEx( "/upload/acrit_exportpro/" );

            DeleteDirFiles( $_SERVER["DOCUMENT_ROOT"]."/local/modules/".$this->MODULE_ID."/install/components", $_SERVER["DOCUMENT_ROOT"]."/bitrix/components/" );
            foreach( $this->siteArray as $sireDir ){
                DeleteDirFiles( $_SERVER["DOCUMENT_ROOT"]."/local/modules/".$this->MODULE_ID."/install/public", $sireDir );
            }
        }
