...
class CIBlockXMLFile
{
	...
	*/
	...
	/*
	Читает часть XML-данных.

    hFile - дескриптор файла, открытый функцией fopen для чтения
    NS - будут заполнены членами
    Параметр charset используется для перекодировки содержимого файла при необходимости.
    Параметры element_stack сохраняют стек разбора родительских элементов дерева xml.
    Параметры file_position отмечают текущую позицию файла.
    time_limit - продолжительность одного шага в секундах.

    NS должны быть сохранены между шагами.
    Они автоматически извлекаются из файла xml и не должны изменяться!
	*/
