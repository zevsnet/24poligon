...
$catalog_id = \Bitrix\Main\Config\Option::get("aspro.max", "CATALOG_IBLOCK_ID",
    CMaxCache::$arIBlocks[SITE_ID]['aspro_max_catalog']['aspro_max_catalog'][0]);
...
$arSections = CMaxCache::CIBlockSection_GetList(array(
    'SORT' => 'ASC',
    'ID' => 'ASC',
    'CACHE' => array('TAG' => CMaxCache::GetIBlockCacheTag($catalog_id), 'GROUP' => array('ID'))
), CMax::makeSectionFilterInRegion($arSectionsFilter), false, array(
    "ID",
    "SORT",
    "IBLOCK_ID",
    "NAME",
...
if ($arSections) {
    ...
    foreach ($arSections as $ID => $arSection) {
        $arSections[$ID]['SELECTED'] = CMenu::IsItemSelected($arSection['SECTION_PAGE_URL'], $cur_page,
            $cur_page_no_index);
        if ($arSection['UF_CATALOG_ICON']) {
            $img = CFile::ResizeImageGet($arSection['UF_CATALOG_ICON'], Array('width' => 36, 'height' => 36),
        ...
        if ($catalog_id) {
            if ($arCatalogIblock = CMaxCache::$arIBlocksInfo[$catalog_id]) {
                if ($catalogPageUrl = str_replace('/', SITE_DIR, $arCatalogIblock['LIST_PAGE_URL'])) {
                    ...
                    if ($menuIblockId) {
                        $menuRootCatalogSectionId = CMaxCache::CIblockSection_GetList(array(
                            'SORT' => 'ASC',
                            'CACHE' => array(
    ...
    if ($bRightBrand) {
        if ($arBrandsID) {
            ...
            $arBrands = CMaxCache::CIblockElement_GetList(array(
                "SORT" => "ASC",
                "CACHE" => array("GROUP" => 'ID', "TAG" => CMaxCache::GetIBlockCacheTag($brandIblockId))
            ), $arBrandFilter, false, false, $arBrandSelect);
