...
if (!empty($arResult["ELEMENTS"]) && CModule::IncludeModule("iblock"))
{
	...
	$bHideNotAvailable = (\Bitrix\Main\Config\Option::get('aspro.max', 'SEARCH_HIDE_NOT_AVAILABLE', 'N') == 'Y');
	...
	while($arElement = $rsElements->Fetch())
	{
		...
		if($GLOBALS['arTheme']['USE_REGIONALITY']['VALUE'] === 'Y' && $GLOBALS['arTheme']['USE_REGIONALITY']['DEPENDENT_PARAMS']['REGIONALITY_FILTER_ITEM']['VALUE'] === 'Y'){
			if($arSectionsIds_NotInRegion = CMax::getSectionsIds_NotInRegion($arElement["IBLOCK_ID"])){
		...
		if($arOffers){
			...
			$arResult["ELEMENTS"][$arElement["ID"]]["MIN_PRICE"]=CMax::getMinPriceFromOffersExt(
					$arOffers,
