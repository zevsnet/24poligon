...
if($arResult['ITEMS']){?>
	...
	<div id="<?=$injectId?>_items" class="bigdata_recommended_products_items">
		...
		<?endif;?>
		<div class="block-items<?=($bRow ? ' flexbox flexbox--row flex-wrap' : '');?><?=($bSlide ? ' owl-carousel owl-theme owl-bg-nav short-nav hidden-dots' : '');?>"<?if($bSlide):?>data-plugin-options='{"nav": true, "autoplay" : false, "autoplayTimeout" : "1000", "margin": -1, "smartSpeed":1000, <?=(count($arResult["ITEMS"]) > 4 ? "\"loop\": true," : "")?> "responsiveClass": true, "responsive":{"0":{"items": 1},"600":{"items": 2},"768":{"items": 3},"992":{"items": 4}}}'<?endif;?>>
