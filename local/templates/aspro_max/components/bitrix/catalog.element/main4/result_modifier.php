...
$productSlider = \SB\Site\SB_CMax::getSliderForItemExt($arResult, $arParams['ADD_PICT_PROP'], 'Y' == $arParams['ADD_DETAIL_TO_SLIDER'],false,450);
...
if($productSlider){
	foreach($productSlider as $i => $arImage){
		$productSlider[$i] = array_merge(
			$arImage, array(
				"BIG" =>  \SB\Site\General::getarWaterMark(2000, $arImage["ID"]),
				"SMALL" => \SB\Site\General::getarWaterMark($arSizePict[$arParams["PICTURE_RATIO"]][0], $arImage["ID"]),
...
if ($arResult['CATALOG'] && isset($arResult['OFFERS']) && !empty($arResult['OFFERS']))
{
	...
	foreach ($arResult['OFFERS'] as $keyOffer => $arOffer)
	{
		...
		$offerSlider = \SB\Site\SB_CMax::getSliderForItemExt($arOffer, $arParams['OFFER_ADD_PICT_PROP'], true); // $arParams['ADD_DETAIL_TO_SLIDER'] == 'Y'
