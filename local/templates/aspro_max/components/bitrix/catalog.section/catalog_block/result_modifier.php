...
if (!empty($arResult['ITEMS'])) {
    ...
    if ($arResult['MODULES']['catalog']) {
        ...
        if ($boolSKU && $featureProps = \Bitrix\Iblock\Model\PropertyFeature::getListPageShowPropertyCodes($arSKU["IBLOCK_ID"],
                array('CODE' => 'Y'))) {
    ...
    foreach ($arResult['ITEMS'] as $key => $arItem) {
        ...
        $arItem['GALLERY'] = SB_CMax::getSliderForItemExt($arItem, $arParams['ADD_PICT_PROP'],
            'Y' == $arParams['ADD_DETAIL_TO_SLIDER'], false, 219);
        ...
        if ($arItem['CATALOG'] && isset($arItem['OFFERS']) && !empty($arItem['OFFERS'])) {
            ...
            foreach ($arItem['OFFERS'] as $keyOffer => $arOffer) {
                ...
                $offerPictures = CIBlockPriceTools::getDoublePicturesForItem($arOffer,
                ...
                if ($arParams["USE_MAIN_ELEMENT_SECTION"] != "Y") {
                    if ($arOffer["DETAIL_PAGE_URL"]) {
                        ...
                        $arOffer["DETAIL_PAGE_URL"] = str_replace($arTmpUrl[0], $arItem["DETAIL_PAGE_URL"],
            ...
            foreach ($arSKUPropIDs as $propkey => $strOneCode) {
                ...
                if ($arPropSKU[$strOneCode]) {
                    Collection::sortByColumn($arPropSKU[$strOneCode], array(
                        "SORT" => array(SORT_NUMERIC, SORT_ASC),
                        "NAME" => array(SORT_NUMERIC, SORT_ASC)
            ...
            if (!$arItem['OFFERS_PROP']) {
                ...
                foreach ($arItem['OFFERS'] as $keyOffer => $arOffer) {
                    ...
                    $arAddToBasketData = CMax::GetAddToBasketArray($arOffer, $totalCount, $arParams["DEFAULT_COUNT"],
                        $arParams["BASKET_URL"], false, $arItemIDs["ALL_ITEM_IDS"], 'small read_more1', $arParams);
                    $arAddToBasketData["HTML"] = str_replace('data-item',
                        'data-props="' . $arOfferProps . '" data-item', $arAddToBasketData["HTML"]);
                    ...
                    if ($arParams['SHOW_GALLERY'] == 'Y') {
                        $arItem['OFFERS'][$keyOffer]['GALLERY'] = CMax::getSliderForItemExt($arOffer,
                            $arParams['OFFER_ADD_PICT_PROP'], true);

                        if ($arItem['GALLERY']) {
                            $arItem['OFFERS'][$keyOffer]['GALLERY'] = array_merge($arItem['OFFERS'][$keyOffer]['GALLERY'],
                                $arItem['GALLERY']);
                        }
...
$arResult['MIN_PRICE_SECTION'] = $arResult['ITEMS'][0]['MIN_PRICE']['VALUE'];
$arResult['MAX_PRICE_SECTION'] = $arResult['ITEMS'][0]['MIN_PRICE']['VALUE'];
foreach ($arResult['ITEMS'] as $ITEM) {
    if( $arResult['MIN_PRICE_SECTION'] > $ITEM['MIN_PRICE']['VALUE']){
        $arResult['MIN_PRICE_SECTION'] = $ITEM['MIN_PRICE']['VALUE'];
    }
    if( $arResult['MAX_PRICE_SECTION'] < $ITEM['MIN_PRICE']['VALUE']){
        $arResult['MAX_PRICE_SECTION'] = $ITEM['MIN_PRICE']['VALUE'];
    }
}
$arResult['COUNT_ELEMENT_SECTION'] = $arResult['NAV_RESULT']->NavRecordCount;
//$arParams['SECTION_ID']
$ob = new CIBlockSection;
$ob->Update($arParams['SECTION_ID'], [
    'UF_MIN_PRICE_SECTION' => $arResult['MIN_PRICE_SECTION'],
    'UF_MAX_PRICE_SECTION' => $arResult['MAX_PRICE_SECTION'],
    'UF_COUNT' => $arResult['COUNT_ELEMENT_SECTION'],
]);
$this->__component->arResultCacheKeys = array_merge($this->__component->arResultCacheKeys,
    ['MIN_PRICE_SECTION','MAX_PRICE_SECTION', 'COUNT_ELEMENT_SECTION']);
