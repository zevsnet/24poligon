			...
			<div class="list_item_wrapp item_wrap item item-parent clearfix bordered box-shadow">
				<div class="list_item item_info catalog-adaptive flexbox flexbox--row <?=($arItem["OFFERS"] ? 'has-sku' : '')?>" id="<?=$arItemIDs["strMainID"];?>">
					...
					<div class="description_wrapp">
						...
						<?if($bNormalView):?>
							<?\Aspro\Functions\CAsproMaxItem::showDelayCompareBtn($arParams, $arItem, $arAddToBasketData, $totalCount, $bUseSkuProps, 'list', false, false, '_small', $currentSKUID, $currentSKUIBlock);?>
						<?else:?>
							<?\Aspro\Functions\CAsproMaxItem::showDelayCompareBtn($arParams, $arItem, $arAddToBasketData, $totalCount, $bUseSkuProps, 'block', ($arParams['USE_FAST_VIEW'] != 'N'), false, '_small', $currentSKUID, $currentSKUIBlock);?>
						<?endif;?>
					</div>
