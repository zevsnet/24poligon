<? if ($arSeoItem): ?>
    ...
    <? if ($arSeoItem["DETAIL_PICTURE"]): ?>
        <div class="seo_block">
            <img data-src="<?= CFile::GetPath($arSeoItem["DETAIL_PICTURE"]); ?>"
                 src="<?= \Aspro\Functions\CAsproMax::showBlankImg(CFile::GetPath($arSeoItem["DETAIL_PICTURE"])); ?>"
                 alt="" title="" class="img-responsive top-big-img lazy"/>
        </div>
    <? endif; ?>
            ...
            <table class="order-block bordered">
                ...
                <tr>
                    <td class="col-md-9 col-sm-8 col-xs-7 valign">
                        <div class="block-item">
                            <div class="flexbox flexbox--row">
                                <div class="block-item__image icon_sendmessage"><?= CMax::showIconSvg("sendmessage",
                                        SITE_TEMPLATE_PATH . "/images/svg/sendmessage.svg", "", "colored_theme_svg",
                                        true, false); ?></div>
                    ...
                    <td class="col-md-3 col-sm-4 col-xs-5 valign btns-col">
                        <div class="btns">
                            <span><span class="btn btn-default btn-sm animate-load" data-event="jqm"
...
<? $section_pos_top = \Bitrix\Main\Config\Option::get("aspro.max", "TOP_SECTION_DESCRIPTION_POSITION",
    "UF_SECTION_DESCR", SITE_ID); ?>
<? $section_pos_bottom = \Bitrix\Main\Config\Option::get("aspro.max", "BOTTOM_SECTION_DESCRIPTION_POSITION",
    "DESCRIPTION", SITE_ID); ?>
...
<? if ($arParams['AJAX_MODE'] == 'Y' && strpos($_SERVER['REQUEST_URI'], 'bxajaxid') !== false): ?>
    <script type="text/javascript">
        setTimeout(function () {
            ...
            $('.ajax_load .catalog_block .catalog_item_wrapp').sliceHeight({
...
<!--        --><?//\_::d( $arResult["VARIABLES"]["SECTION_ID"]);?>
        ...
        <? if ($itemsCnt): ?>
            ...
            if(is_array($MAX_SMART_FILTER)){
//                $MAX_SMART_FILTER = array_merge($MAX_SMART_FILTER,[
////                    [
////                        "LOGIC"=>'OR',
////                        ">CATALOG_PRICE_4"=>0,
////                        ">CATALOG_PRICE_8"=>0,
...
//                ]);
                    ...
                    <div class="no_products">
                        <div class="wrap_text_empty">
                            <? if ($_REQUEST["set_filter"]) { ?>
                                <? $APPLICATION->IncludeFile(SITE_DIR . "include/section_no_products_filter.php",
                                    Array(), Array("MODE" => "html", "NAME" => GetMessage('EMPTY_CATALOG_DESCR'))); ?>
                            <? } else { ?>
                                <? $APPLICATION->IncludeFile(SITE_DIR . "include/section_no_products.php", Array(),
                                    Array("MODE" => "html", "NAME" => GetMessage('EMPTY_CATALOG_DESCR'))); ?>
                            <? } ?>
                        </div>
                    </div>
                </div>
        ...
        <? if ($isAjax == "N") { ?>
            <? if ($arParams["BLOG_IBLOCK_ID"]): ?>
                ...
                $blogsCount = CMaxCache::CIblockElement_GetList(array("CACHE" => array("TAG" => CMaxCache::GetIBlockCacheTag($arParams["BLOG_IBLOCK_ID"]))),
                    $GLOBALS[$filterName], array());
                if ($blogsCount):
                    ?>
                    <div class="linked-blog-list <?= $linkedArticlesPos ?>"
                         data-desktop_row="<?= $linkedArticlesRows ?>"
                         data-mobile_row="<?= $linkedArticlesRowsMobile ?>">
                        <? $APPLICATION->IncludeComponent(
                            "bitrix:news.list",
                            "news-list",
                            array(
	...
	*/ ?>
        ...
        <? else: ?>
            ...
            <? else: ?>
                <?
                $ipropValues = new \Bitrix\Iblock\InheritedProperty\SectionValues($arParams["IBLOCK_ID"],
                    IntVal($arSection["ID"]));
        ...
        if ($arSeoItem) {
            ...
            if ($arSeoItem["IPROPERTY_VALUES"]["ELEMENT_META_DESCRIPTION"]) {
                $APPLICATION->SetPageProperty("description",
                    $arSeoItem["IPROPERTY_VALUES"]["ELEMENT_META_DESCRIPTION"]);
            }
