...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		init: function(parameters)
		{
			...
			this.imgFolder = parameters.imgFolder || '';
			...
			if (this.totalBlockNode)
			{
				...
				this.totalInfoBlockDataNode = this.totalBlockNode.querySelector('.bx-soa-cart-total-data');
			...
			BX.addClass(this.basketBlockNode, 'bx-selected');
		...
		sendRequest: function(action, actionData)
		{
			...
			{
				form = BX('bx-soa-tanais-order-form');
				if (form)
				{
						...
						switch (action)
						{
							...
							case 'enterCoupon':
								if (result && result.order)
								{
									this.deliveryCachedInfo = [];
									this.refreshOrder(result);
									BX.Sale.BasketComponent.sendRequest("refreshAjax", {onlyBasketRefresh: true});
								}
								else
								{
		...
		{
			var form = BX('bx-soa-tanais-order-form'),
					...
					events: {
						onAfterPopupShow: BX.delegate(function(){
							...
							this.loadingScreen.popupContainer.appendChild(
								BX.create('IMG', {props: {src: this.imgFolder + '/loader.gif'}})
							);
		...
		showPagination: function(entity, node)
		{
			...
			var pagination, navigation = [], i,
				pageCounter, active, paginationNode;
		...
		initFirstSection: function()
		{
			...
			if(this.basketBlockNode && firstSection.id != this.basketBlockNode.id) {
			...
			for (i in this.locations)
			{
				...
				if (clearButton)
				{
					BX.bind(clearButton, 'click', function(e){
						var target = e.target || e.srcElement,
							parent = BX.findParent(target, {tagName: 'DIV', className: 'tanais-form-group'}),
							locationInput;
		...
		fixLocationsStyle: function(section, hiddenSection)
		{
			...
			if (locationSearchInputs.length)
				for (i = 0; i < locationSearchInputs.length; i++)
					BX.addClass(locationSearchInputs[i], 'tanais-form-control');

			if (locationStepInputs.length)
				for (i = 0; i < locationStepInputs.length; i++)
					BX.addClass(locationStepInputs[i], 'tanais-form-control');
		},
		...
		show: function(node)
		{
			...
			if (this.basketBlockNode && node.id == this.basketBlockNode.id) {
		...
		getBlockFooter: function(node)
		{
			...
			/*node.appendChild(
				BX.create('DIV', {
					...
					children: [
						BX.create('DIV', {
							props: {className: 'bx-soa-more-btn'},
							children: buttons
						})
					]
				})
		...
		changeVisibleSection: function(section, state)
		{
			...
			if (this.basketBlockNode && section.id !== this.basketBlockNode.id)
			{
		...
		editSection: function(section)
		{
			...
			if (this.result.SHOW_AUTH && section.id != this.authBlockNode.id && (this.basketBlockNode && section.id != this.basketBlockNode.id))
		...
		editAuthorizeForm: function(authContent)
		{
			...
			authContent.appendChild(BX.create('DIV', {children: [authFormNode]}));
		},
		...
		editSocialContent: function(authContent)
		{
			...
			authContent.appendChild(BX.create('DIV', {children: nodes}));
		},
		...
		editBasketItemsHeader: function(basketItemsNode)
		{
			...
			basketItemsNode.appendChild(
				BX.create('DIV', {
					props: {className: 'bx-soa-item-tr'},
					children: headers
				})
			);
		},
		...
		popupShow: function(e, url, source)
		{
			...
			this.popup = new BX.PopupWindow('bx-soa-image-popup', null, {
				...
				content: BX.create('DIV', {
					...
					children: [BX.create('IMG', {props: {src: this.imgFolder + "/loader.gif"}})]
				})
			});
			this.popup.show();
		},
		...
		getCouponChangeEvent: function () {
			return BX.delegate(function(event){
					...
					children: [
						BX.create('DIV', {
							...
							children: [
								BX.create('INPUT', {
									props: {
										className: 'tanais-form-control bx-ios-fix',
										type: 'text'
									},
									events: {
										change: this.getCouponChangeEvent()
									}
								}),
								BX.create("SPAN", {
									props: {
										className: 'bx-soa-coupon-btn'
									}
								})
							]
						}),
		...
		getDeliveryLocationInput: function(node)
		{
			...
			if (location && location[0] && location[0].output)
			{
				...
				insertedLoc = BX.create('DIV', {
					...
					props: {className: 'tanais-form-group bx-soa-location-input-container'},
			...
			if (altProperty)
			{
				...
				input = BX.create('INPUT', {
					props: {
						...
						autocomplete: 'city',
						className: 'tanais-form-control bx-soa-customer-input bx-ios-fix',
		...
		getZipLocationInput: function(node)
		{
			...
			if (zipProperty)
			{
				...
				propsItemNode = BX.create('DIV', {props: {className: "tanais-form-group bx-soa-location-input-container"}});
		...
		getPersonTypeControl: function(node)
		{
			...
			{
				input = BX.create('DIV', {
					props: {className: 'tanais-form-group person-type'},
				});
			...
			if (personTypesCount > 2)
			{
				...
				node.appendChild(BX.create('SELECT', {
					props: {name: 'PERSON_TYPE', className: 'tanais-form-control'},
					children: options,
				...
				for (i in this.result.PERSON_TYPE)
				{
					if (this.result.PERSON_TYPE.hasOwnProperty(i))
					{
						...
						node.appendChild(
							BX.create('DIV', {
								props: {
									className: 'tanais-radio-inline' + (currentType.CHECKED == 'Y' ? " checked" : "")
								},
								children: [label]
							})
						);
			...
			if (profilesLength)
			{
				...
				)
				{
					...
					input = BX.create('SELECT', {
						props: {className: 'tanais-form-control', name: 'PROFILE_ID'},
						children: options,
						events:{
							change: BX.delegate(function(){
		...
		createPaySystemItem: function(item)
		{
			...
			itemNode = BX.create('DIV', {
				...
				children: [
					...
					title,
					BX.create('DIV', {
						props: {className: 'bx-soa-pp-company-smalldescr'},
		...
		editDeliveryInfo: function(deliveryNode)
		{
			...
			var deliveryInfoContainer = BX.create('DIV', {props: {className: 'bx-soa-pp-desc-container'}}),
				currentDelivery, title, period,
				infoList, extraServices, extraServicesNode;

			var isShowDeliveryInfo = false;
			var issetPeriod = false;
			...
			title = BX.create('DIV', {
				...
				children: [
					currentDelivery.CALCULATE_DESCRIPTION
						? (isShowDeliveryInfo = true) && BX.create('DIV', {props: {className: 'bx-soa-pp-company-desc'}, html: currentDelivery.CALCULATE_DESCRIPTION})
						: null
				]
			});
			...
			if (currentDelivery.PERIOD_TEXT && currentDelivery.PERIOD_TEXT.length)
			{
				...
				isShowDeliveryInfo = issetPeriod = true;
			}

			infoList = !issetPeriod ? null : BX.create('UL', {props: {className: 'bx-soa-pp-list'}, children: [period]});
			...
			if (isShowDeliveryInfo) {
				deliveryInfoContainer.appendChild(
					BX.create('DIV', {
						props: {className: 'bx-soa-pp-company'},
						children: [title, extraServicesNode, infoList]
					})
				);

				deliveryNode.appendChild(deliveryInfoContainer);

				BX("bx_soa_item_delivery_id_" + currentDelivery.ID).appendChild(deliveryInfoContainer);
			}
			...
			for (i in delivery.EXTRA_SERVICES)
			{
				...
				if (currentService.editControl.indexOf('this.checked') == -1)
				{
					...
					serviceNode = BX.create('DIV', {
						props: {className: 'tanais-form-group bx-soa-pp-field'},
					...
					if (input)
						BX.addClass(input, 'tanais-form-control');
				}
				else
				{
					serviceNode = BX.create('DIV', {
		...
		createDeliveryItem: function(item)
		{
			...
			var hasDeliveryCost = false;

			if (item.PRICE >= 0 || typeof item.DELIVERY_DISCOUNT_PRICE !== 'undefined')
			{
				var actualPrice = typeof item.DELIVERY_DISCOUNT_PRICE !== 'undefined' ? 
					(item.DELIVERY_DISCOUNT_PRICE > 0 ? item.DELIVERY_DISCOUNT_PRICE_FORMATED : this.params.MESS_PRICE_FREE) :
					(item.PRICE > 0 ? item.PRICE_FORMATED : this.params.MESS_PRICE_FREE);
			...
			{
				var actualPrice = typeof deliveryCached.DELIVERY_DISCOUNT_PRICE !== 'undefined' ? 
					(deliveryCached.DELIVERY_DISCOUNT_PRICE > 0 ? deliveryCached.DELIVERY_DISCOUNT_PRICE_FORMATED : this.params.MESS_PRICE_FREE) :
					(deliveryCached.PRICE > 0 ? deliveryCached.PRICE_FORMATED : this.params.MESS_PRICE_FREE);
			...
			label = BX.create('DIV', {
				props: {
					className: 'bx-soa-pp-company-graf-container' + (hasDeliveryCost ? ' has-delivery-cost' : '')
			...
			itemNode = BX.create('DIV', {
				props: {
					id: 'bx_soa_item_delivery_id_' + deliveryId,
					className: 'bx-soa-pp-company'
				},
				children: [
					BX.create('DIV', {
						props: {
							className: 'bx-soa-pp-company-inner-block'
						},
						children: [
							label,
							BX.create('DIV', {
								props: {className: 'bx-soa-pp-company-smalldescr' + (checked ? ' bx-soa-pp-company-desc' : '')},
								html: item.DESCRIPTION
							})
						],
		...
		editFadeDeliveryContent: function(node)
		{
			...
			if (selectedDelivery && selectedDelivery.NAME)
			{
				...
				node.appendChild(
					BX.create('DIV', {
						props: {className: 'bx-soa-pp-company-selected'},
						children: arNodes
					})
				);
				node.appendChild(
					BX.create('DIV', {
						props: {className: 'bx-soa-pp-price'},
		...
		activatePickUp: function(deliveryName)
		{
			...
			this.pickUpBlockNode.querySelector('h2.bx-soa-section-title .bx-soa-section-title-text').innerHTML =
		...
		editPickUpList: function(isNew)
		{
			...
			if (!!isNew)
			{
				container = this.pickUpHiddenBlockNode.querySelector('.bx_soa_pickup>div');
				if (!container)
					container = this.pickUpBlockNode.querySelector('.bx_soa_pickup>div');
			...
			storeNode = BX.create('DIV', {
				...
				children: [
					...
					BX.create('DIV', {
						...
						children: [
							BX.create('A', {
								props: {href: 'javascript:void(0);', className: 'btn btn-default'},
		...
		{
			var deliveries = [],
				delivery_sort = this.params.ORDER_DELIVERY_SORT,
				problemDeliveries = [],
				...
				sortFuncByPrice = function(a, b) {
					var sort;
					if (delivery_sort == "PRICE_DESC") 
						sort = parseInt(b.PRICE) - parseInt(a.PRICE);
					else
						sort = parseInt(a.PRICE) - parseInt(b.PRICE);
					if (sort === 0)
					{
						return a.OWN_NAME.toLowerCase() > b.OWN_NAME.toLowerCase()
							? 1
			...
			deliveries.sort(
				this.params.DELIVERY_NO_AJAX == "Y" ? 
					(delivery_sort !== "SORT" ? sortFuncByPrice : sortFunc) : sortFunc
			);
		...
		getPropertyRowNode: function(property, propsItemsContainer, disabled)
		{
			...
			{
				BX.addClass(propsItemNode, "tanais-form-group bx-soa-customer-field");
		...
		insertStringProperty: function(property, propsItemNode, disabled)
		{
			...
			else
			{
				...
				var propSettings = property.getSettings();

				if (
					this.params.USE_LEGAL_ENTITY_AUTOLOAD_DATA == "Y" &&
					this.params.AUTOLOAD_LEGAL_INN.length > 0 &&
					this.params.AUTOLOAD_LEGAL_INN == propSettings["CODE"]
				) {
					BX.addClass(propContainer, "has-soa-action-button");
					...
					propContainer.appendChild(
						BX.create('A', {
							...
							html: this.params.MESS_AUTOLOAD_LEGAL_INN_BTN,
							events: {
								click: BX.proxy(this.autoLoadLegalDataByINN, this)
							}
						})
					);
				}
				...
				if (
					this.params.USE_ADDR_AUTOCOMPLATE == "Y" &&
					this.params.AUTOCOMPLATE_ADDR.length > 0 &&
					this.params.AUTOCOMPLATE_ADDR == propSettings["CODE"]
				) {
					BX.addClass(propContainer, "has-soa-address-suggest");

					propContainer.appendChild(
						BX.create('SPAN', {
							props: {
								className: 'soa-suggests-container'
							},
							children: [
								BX.create('SPAN', {
									props: {
										className: 'soa-suggests-block'
									}
								})
							]
						})
					...
					property.addEvent("input", this.autocompleteAddr);
					property.addEvent("blur", function (e) {
						$(this).next(".soa-suggests-container").fadeOut();
					});
					property.addEvent("focus", function (e) {
						var _sc = $(this).next(".soa-suggests-container");
						var _scb = _sc.find(".soa-suggests-block");
						if (_scb.length > 0 && _scb.html() !== "") {
							_sc.fadeIn();
						}
					});
				}
		...
		alterProperty: function(settings, propContainer)
		{
			...
			if (textNode)
			{
				...
				if (settings.IS_ADDRESS == 'Y')
					textNode.setAttribute('autocomplete', 'new-password');
				...
				if (settings.IS_PHONE == 'Y') {
					...
					$(textNode).mask(this.params.PHONE_INPUT_MASK);
				}
			...
			for (i = 0; i < inputs.length; i++)
			{
				...
				BX.addClass(inputs[i], 'tanais-form-control bx-soa-customer-input bx-ios-fix');
			}

			inputs = propContainer.querySelectorAll('select');
			for (i = 0; i < inputs.length; i++)
				BX.addClass(inputs[i], 'tanais-form-control');

			inputs = propContainer.querySelectorAll('textarea');
			for (i = 0; i < inputs.length; i++)
			{
				inputs[i].placeholder = settings.DESCRIPTION;
				BX.addClass(inputs[i], 'tanais-form-control bx-ios-fix');
			}
			...
			{
				BX.addClass(add[i], 'btn btn-default');
			...
			if (add.length)
			{
				...
				BX.bind(add, 'click', BX.delegate(function(e){
					...
					if (add.length && add[add.length - 2])
					{
						...
						BX.addClass(add[add.length - 2], 'btn btn-default');
					}
					...
					if (textInputs.length)
					{
						...
						BX.addClass(textInputs[textInputs.length - 1], 'tanais-form-control bx-soa-customer-input bx-ios-fix');
					...
					if (textAreas.length)
					{
						...
						BX.addClass(textAreas[textAreas.length - 1], 'tanais-form-control bx-ios-fix');
					}
			...
			if (propsErrors.length && !navigated)
			{
				if (this.activeSectionId == this.propsBlockNode.id)
				{
					...
					for (i = 0; i < tooltips.length; i++)
					{
						...
						{
							this.animateScrollTo(BX.findParent(tooltips[i], {className: 'tanais-form-group bx-soa-customer-field'}), 800, 50);
							break;
						}
					}
				}
		...
		showValidationResult: function(inputs, errors)
		{
			...
			var input0 = BX.type.isElementNode(inputs[0]) ? inputs[0] : inputs[0][0],
				formGroup = BX.findParent(input0, {tagName: 'DIV', className: 'tanais-form-group'}),
			...
			for (i = 0; i < inputs.length; i++)
			{
				inputDiv = BX.findParent(inputs[i], {tagName: 'DIV', className: 'tanais-form-group'});
		...
		validateLocation: function(input, arProperty, fieldName)
		{
			...
			var parent = BX.findParent(input, {tagName: 'DIV', className: 'tanais-form-group'}),
		...
		{
			var propsMapContainer = BX.create('DIV', {style: {marginBottom: '10px'}}),
		...
		editPropsComment: function(propsNode)
		{
			...
			input = BX.create('TEXTAREA', {
				props: {
					id: 'orderDescription',
					cols: '4',
					className: 'tanais-form-control bx-soa-customer-textarea bx-ios-fix tanais-textarea',
					name: 'ORDER_DESCRIPTION'
				},
				text: this.result.ORDER_DESCRIPTION ? this.result.ORDER_DESCRIPTION : ''
			});
			div = BX.create('DIV', {
				props: {className: 'tanais-form-group bx-soa-customer-field'},
		...
		{
			if (!this.totalInfoBlockDataNode || !this.result.TOTAL)
				return;
			...
			BX.cleanNode(this.totalInfoBlockDataNode);
			...
			{
				priceHtml = '<span class="bx-price-old">' + 
					total.PRICE_WITHOUT_DISCOUNT + '</span><br>' + '<span class="with-discount-value">' + priceHtml + '</span>';
			}

			this.totalInfoBlockDataNode.appendChild(this.createTotalUnit(total.BASKET_POSITIONS + this.declOfNum(total.BASKET_POSITIONS, [' товар', ' товара', ' товаров']), priceHtml, params));

			if (this.options.showOrderWeight)
			{
				this.totalInfoBlockDataNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_WEIGHT_SUM'), total.ORDER_WEIGHT_FORMATED));
			}
			...
			if (this.options.showTaxList)
			{
				for (i = 0; i < total.TAX_LIST.length; i++)
				{
					...
					this.totalInfoBlockDataNode.appendChild(
			...
			else
			{
				...
				{
					deliveryValue = '<span class="bx-price-old">' + curDelivery.PRICE_FORMATED + '</span><br>' + '<span class="with-discount-value">' + deliveryValue + '</span>';
				}
			}

			if (this.result.DELIVERY.length)
			{
				this.totalInfoBlockDataNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_DELIVERY'), deliveryValue, params));
			}
			...
			if (this.options.showDiscountPrice)
			{
				...
				this.totalInfoBlockDataNode.appendChild(this.createTotalUnit(discText + ':', total.DISCOUNT_PRICE_FORMATED, {highlighted: true}));
			}

			if (this.options.showPayedFromInnerBudget)
			{
				this.totalInfoBlockDataNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_IT'), total.ORDER_TOTAL_PRICE_FORMATED));
				this.totalInfoBlockDataNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_PAYED'), total.PAYED_FROM_ACCOUNT_FORMATED));
				this.totalInfoBlockDataNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_LEFT_TO_PAY'), total.ORDER_TOTAL_LEFT_TO_PAY_FORMATED, {total: true}));
			}
			else
			{
				this.totalInfoBlockDataNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_IT'), total.ORDER_TOTAL_PRICE_FORMATED, {total: true}));
			}

			if (parseFloat(total.PAY_SYSTEM_PRICE) >= 0 && this.result.DELIVERY.length)
			{
				this.totalInfoBlockDataNode.appendChild(this.createTotalUnit(BX.message('SOA_PAYSYSTEM_PRICE'), '~' + total.PAY_SYSTEM_PRICE_FORMATTED));
			}

			var couponContainer = BX.create('DIV', {
				props: {
					className: 'bx-total-coupon-container'
				}
			});

			if (this.params.SHOW_COUPONS == 'Y') {
				this.editCoupons(couponContainer);
			}

			this.totalInfoBlockDataNode.appendChild(couponContainer);
			...
			if (!this.result.SHOW_AUTH)
			{
				var _children = [
					BX.create('A', {
						props: {
							href: 'javascript:void(0)',
							className: 'btn btn-default btn-order-save'
						},
				...
				if (this.params.USE_BUY_ONE_CLICK == "Y") {
					_children.push(
						BX.create('A', {
							props: {
								href: 'javascript:void(0)',
		...
		editMobileTotalBlock: function()
		{
			...
			if (this.params.USE_BUY_ONE_CLICK == "Y") {
				BX.bind(this.mobileTotalBlockNode.querySelector('.bx-soa-cart-total .btn-order-one-click'), 'click', BX.proxy(this.buyOneClickFormShow, this));
			}

			BX.bind(
				this.mobileTotalBlockNode.querySelector(".bx-soa-coupon-input input[type='text']"),
				'change',
				this.getCouponChangeEvent()
			);
		},
		...
		buyOneClickFormHide: function () {
			$(".bx-soa-cart-total-data").addClass("animate__fadeInBlock").show();
			$(".bx-soa-cart-total-form").removeClass("animate__fadeInBlock").hide();
		},

		buyOneClickFormShow: function () {
			$(".bx-soa-cart-total-data").removeClass("animate__fadeInBlock").hide();
			$(".bx-soa-cart-total-form").addClass("animate__fadeInBlock").show();
		},

		buyOneClickFormSend: function (btn) {
			var oneClickForm = $(btn).closest(".bx-soa-cart-total-form");
			var msgBox = oneClickForm.find(".bx-soa-cart-total-form-message-box");
			var requiredInputs = oneClickForm.find(":required");
			var _this = this;
			...
			var fields = {
				"full_name": oneClickForm.find("[name='full_name']").val(),
				"phone": oneClickForm.find("[name='phone']").val(),
				"email": oneClickForm.find("[name='email']").val(),
				"sessid": BX.bitrix_sessid()
			};

			var comment = oneClickForm.find("[name='comment']");
			var captcha_code = oneClickForm.find("[name='captcha_code']");
			var captcha_word = oneClickForm.find("[name='captcha_word']");
			...
			if (captcha_code.length > 0) {
				fields["captcha_code"] = captcha_code.val();
				fields["captcha_word"] = captcha_word.val();
			}
			...
			if (isValidForm)
			{
				...
				$.post(
					this.templateFolder + "/ajax/one_click_order_ajax.php",
					fields,
					function (result)
					{
						if (result.success) {
							window.location.href = _this.params.CURRENT_PAGE + "?ORDER_ID=" + result.data.ORDER_ID;
						}
						else {
							msgBox.addClass("error").html("");

							if (result.errors.length < 1) {
								result.errors.push(_this.params.MESS_TOTAL_BLOCK_BUY_ONE_CLICK_FORM_UNKNOWN_ERROR);
							}
		...
		autoLoadLegalDataByINN: function (btn) {
			if (
				this.params.USE_LEGAL_ENTITY_AUTOLOAD_DATA == "Y" &&
				this.params.AUTOLOAD_LEGAL_INN.length > 0
			) {
				var inn = document.querySelector(
					".tanais-order #soa-property-" + 
					this.getOrderPropertyIdByCode(this.params.AUTOLOAD_LEGAL_INN)
				);
				...
				if (inn !== null && inn.value.length > 0)
				{
					...
					$.post(
						this.templateFolder + "/ajax/get_data_for_legal_inn_ajax.php",
						{
							"inn": inn.value,
							"sessid": BX.bitrix_sessid()
						},
						function (result)
						{
							var orderAjaxComponent = BX.Sale.OrderAjaxComponent;

							var kpp = document.querySelector(
								".tanais-order #soa-property-" + 
								orderAjaxComponent.getOrderPropertyIdByCode(
									orderAjaxComponent.params.AUTOLOAD_LEGAL_KPP
								)
							);

							var companyAddr = document.querySelector(
								".tanais-order #soa-property-" + 
								orderAjaxComponent.getOrderPropertyIdByCode(
									orderAjaxComponent.params.AUTOLOAD_LEGAL_COMPANY_ADDR
								)
							);
							...
							var company = document.querySelector(
								".tanais-order #soa-property-" + 
								orderAjaxComponent.getOrderPropertyIdByCode(
									orderAjaxComponent.params.AUTOLOAD_LEGAL_COMPANY
								)
							);
							...
							if (result.success)
							{
								...
								if (kpp !== null && (typeof resultData.data.kpp !== "undefined")) {
									kpp.value = resultData.data.kpp;
								}

								if (companyAddr !== null && (typeof resultData.data.address.value !== "undefined")) {
									companyAddr.value = resultData.data.address.value;
								}

								if (company !== null && (typeof resultData.value  !== "undefined")) {
									company.value = resultData.value;
								}
							}
							else
							{
								if (kpp !== null) kpp.value = "";
								if (companyAddr !== null) companyAddr.value = "";
								if (company !== null) company.value = "";

								console.log(result.errors);
							}

							setTimeout(function () {
								orderAjaxComponent.endLoader();
							}, 400);
						},
						"json"
					);
		...
		autocompleteAddr: function ()
		{
			...
			if (_value.length > 1) {
				$.post(
					BX.Sale.OrderAjaxComponent.templateFolder + "/ajax/get_addr_suggest.php",
					{
						"addr": _value,
						"sessid": BX.bitrix_sessid()
					},
					function (result)
					{
						if (result.success)
						{
							...
							for (i in result.data) {
								...
								items.push("<span>" + result.data[i].value + "</span>");
							}

							$(_this).next(".soa-suggests-container")
								.fadeIn()
								.find(".soa-suggests-block")
								.html(items.join(""))
								.find("span")
								.on("click", function () {
									_this.value = $(this).text();
									$(this).closest(".soa-suggests-block").html("");
									$(this).closest(".soa-suggests-container").fadeOut();
								});
						}
						else {
							console.log(result.errors);
		...
		getOrderPropertyIdByCode: function (code)
		{
			for (var i = 0; i < this.result.ORDER_PROP.properties.length; i++) {
				if (this.result.ORDER_PROP.properties[i]["CODE"] == code) {
					return this.result.ORDER_PROP.properties[i]["ID"];
				}
			}
...
$(function () {
	...
	if (typeof BX.Sale.OrderAjaxComponent.params !== "undefined") {
		$(".tanais-order .bx-soa-cart-total-form [autocomplete='tel']").mask(
			BX.Sale.OrderAjaxComponent.params.PHONE_INPUT_MASK
		);
	}
