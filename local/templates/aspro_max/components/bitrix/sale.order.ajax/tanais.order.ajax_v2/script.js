BX.saleOrderAjax = { // bad solution, actually, a singleton at the page
	...
	init: function(options)
	{
		...
		this.controls.scope = BX('bx-soa-tanais-order');
		...
		BX.bindDelegate(this.controls.scope, 'click', {className: '-bx-popup-set-mode-add-loc'}, function(){
			...
			BX.prepend(input, BX('bx-soa-tanais-order'));
...
$(function () {
	$(".soa-action-save-to-pdf").on("click", function () {
		window.location.href = BX.Sale.OrderAjaxComponent.templateFolder + "/ajax/spec_ajax.php?type=pdf";
	});

	$(".soa-action-print").on("click", function () {
		$.get(
			BX.Sale.OrderAjaxComponent.templateFolder + "/ajax/spec_ajax.php",
			{
				"type": "html"
			},
			function (resultHtml) {
