...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		initializePrimaryFields: function()
		{
			...
			this.deliveryPickupId = [];
			this.deliveryPickupDefaultType = 'delivery';//pickup - для выбора вкладки Самовывоз
			this.sbSelectedIdDelivery = null;
			this.sbSelectedIdDeliveryOld = null;
		...
		init: function(parameters)
		{
			...
			this.deliveryPickupId = parameters.deliveryPickupId;
			this.deliveryPickupDefaultType = parameters.deliveryPickupDefaultType;
			...
			this.typeBlockNode = BX(parameters.typeBlockId);
			this.typeHiddenBlockNode = BX(parameters.typeBlockId + '-hidden');
			...
			else
			{
				BX.ajax({
					...
					onsuccess: BX.delegate(function(result) {
						...
						let this_ = this;
						Object.keys(result.order.DELIVERY).forEach(function(key,val) {
							if(result.order.DELIVERY[key]['CHECKED'] == 'Y'){
								this_.sbSelectedIdDeliveryOld = result.order.DELIVERY[key]['ID']
							}
						},this_);
						...
						$('[name="ORDER_PROP_47"],[name="ORDER_PROP_46"],[name="ORDER_PROP_48"]').suggestions({
							token: "6e6f1d6e075d3c668e91949b9d4bf0c8183e61f4",
							type: "NAME",
							onSelect: function(suggestion) {
							}
						});
						$('[name="ORDER_PROP_40"]').suggestions({
							token: "6e6f1d6e075d3c668e91949b9d4bf0c8183e61f4",
							type: "EMAIL",
							onSelect: function(suggestion) {
							}
						});
						$('[name="ORDER_PROP_51"]').suggestions({
							token: "6e6f1d6e075d3c668e91949b9d4bf0c8183e61f4",
							type: "ADDRESS",
							count: 5,
							/* Вызывается, когда пользователь выбирает одну из подсказок */
							onSelect: function (suggestion) {
								$('[name="ORDER_PROP_42"]').val(suggestion.data.postal_code);
								//$("#ZIP_PROPERTY_CHANGED").val(suggestion.data.postal_code);
							}
						});
		...
		getData: function(action, actionData)
		{
			var data = {
				...
				SITE_ID: this.siteId,
				signedParamsString: this.signedParamsString,
				sbSelectedIdDelivery: this.sbSelectedIdDelivery,
				sbSelectedIdDeliveryOld: this.sbSelectedIdDeliveryOld,
			};
			...
			for (k in errors)
			{
				...
				switch (k.toUpperCase())
				{
					...
					case 'AUTH':
						...
						break;
					case 'TYPE':
						if (showAll || this.typeBlockNode.getAttribute('data-visited') === 'true')
						{
							this.showError(this.typeBlockNode, blockErrors, true);
							this.showError(this.typeHiddenBlockNode, blockErrors);
						}
						break;
					case 'REGION':
		...
		showBlockErrors: function(node)
		{
			...
			switch (node.id)
			{
				case this.typeBlockNode.id:
					hiddenNode = this.typeHiddenBlockNode;
					errors = this.result.ERROR.TYPE;
					break;
		...
		getGoalId: function(goal, section)
		{
			...
			switch (section.id)
			{
				case this.basketBlockNode.id:
					goalId = isEdit ? this.params.YM_GOALS_EDIT_BASKET : this.params.YM_GOALS_MAX_BASKET; break;
				case this.regionBlockNode.id:
					goalId = isEdit ? this.params.YM_GOALS_EDIT_REGION : this.params.YM_GOALS_MAX_REGION; break;
				case this.paySystemBlockNode.id:
					goalId = isEdit ? this.params.YM_GOALS_EDIT_PAY_SYSTEM : this.params.YM_GOALS_MAX_PAY_SYSTEM; break;
				case this.deliveryBlockNode.id:
					goalId = isEdit ? this.params.YM_GOALS_EDIT_DELIVERY : this.params.YM_GOALS_MAX_DELIVERY; break;
				case this.pickUpBlockNode.id:
					goalId = isEdit ? this.params.YM_GOALS_EDIT_PICKUP : this.params.YM_GOALS_MAX_PICKUP; break;
				case this.propsBlockNode.id:
					goalId = isEdit ? this.params.YM_GOALS_EDIT_PROPERTIES : this.params.YM_GOALS_MAX_PROPERTIES; break;
			}
...
debugger
		...
		getBlockFooter: function(node)
		{
			...
			if(currentSection.id == this.basketBlockNode.id){
		...
		editSection: function(section)
		{
			...
			if(section.id == this.basketBlockNode.id){
				...
				if (this.result.SHOW_AUTH)
				{
					editButton && BX.bind(editButton, 'click', BX.delegate(function(){
						this.animateScrollTo(this.authBlockNode);
		...
		editTypeBlock: function(active)
		{
			if (!this.typeBlockNode)
				return;

			if (active)
			{
				this.editActiveTypeBlock(true);
		...
		editActiveTypeBlock: function(activeNodeMode)
		{
			var node = activeNodeMode ? this.typeBlockNode : this.typeHiddenBlockNode,
				regionContent, regionNode, regionNodeCol,regionNodeCol2;
debugger
			...
			else
			{
				...
				regionNodeCol = BX.create('DIV', {props: {className: 'col-xs-12 col-md-12'}});
		...
		editActiveRegionBlock: function(activeNodeMode)
		{
			...
			else
			{
				...
				regionNode = BX.create('DIV', {props: {className: 'bx_soa_location row'}});
				regionNodeCol = BX.create('DIV', {props: {className: 'col-xs-12 col-md-8'}});


				this.getProfilesControl(regionNodeCol);

				this.getDeliveryLocationInput(regionNodeCol);
				regionNodeCol2 = BX.create('DIV', {props: {className: 'col-xs-12 col-md-4'}});
				this.getZipLocationInput(regionNodeCol2);
				if (!this.result.SHOW_AUTH)
				{
				...
				regionNode.appendChild(regionNodeCol);
				regionNode.appendChild(regionNodeCol2);
				regionContent.appendChild(regionNode);
				this.getBlockFooter(regionContent);
			}
		},
		...
		editPaySystemItems: function(paySystemNode)
		{
			...
			var paySystemItemsDescription = BX.create('DIV', {props: {className: 'col-sm-12 bx-soa-pp-company-description'}});
		...
		createPaySystemItem: function(item)
		{
			var checked = item.CHECKED == 'Y',
				...
				title, label, itemNode, injectNode;
			...
			itemNode = BX.create('DIV', {
				...
				props: {className: 'sb_block_pay bx-soa-pp-company bx-soa-pp-company-item col-lg-4 col-sm-4 col-xs-6'},
		...
		getInnerPaySystem: function()
		{
			...
			var accountOnly = this.params.ONLY_FULL_PAY_FROM_ACCOUNT && (this.params.ONLY_FULL_PAY_FROM_ACCOUNT == 'Y'),
				...
				logotype, logoNode,subTitle, label, title, hiddenInput, htmlString, innerPsDesc, balanceString;
			...
			label = BX.create('LABEL', {
				props: {className: 'bx-soa-pp-company-subTitle' + (isSelected ? ' checked' : '')},
				text: paySystem.NAME
			});

			label.appendChild(BX.create('INPUT', {
				props: {
					type: 'checkbox',
					className: 'bx-soa-pp-company-checkbox',
					name: 'PAY_CURRENT_ACCOUNT',
			...
			balanceString = BX.util.htmlspecialchars(this.params.MESS_INNER_PS_BALANCE + ' ' + this.result.CURRENT_BUDGET_FORMATED);
			label.setAttribute('title', balanceString);

			subTitle = BX.create('DIV', {
				props: {className: 'checkbox' + (isSelected ? ' checked' : '')},
				children: [label]
			});
		...
		editActiveDeliveryBlock: function(activeNodeMode)
		{
			var node = activeNodeMode ? this.deliveryBlockNode : this.deliveryHiddenBlockNode,
				deliveryContent,deliveryTitle, deliveryNode,
				deliveryTypeContainer = BX.create('DIV', {props: {className: 'bx-soa-pp-item-container'}});
			...
			{
				deliveryTitle = node.querySelector('.bx-soa-section-title-container');
				...
				var typeSamovivoz = BX.create('DIV', {
					attrs: {'type': 'pickup'},
					props: {className: (this.deliveryPickupDefaultType == 'pickup') ? 'sb_type_delivery active' : 'sb_type_delivery' },
					html:'Самовывоз',
					events: {click: BX.proxy(this.selectTypeDelivery, this)}
				});
				var typeDelivery = BX.create('DIV', {
					attrs: {'type': 'delivery'},
					props: {className: (this.deliveryPickupDefaultType != 'pickup') ? 'sb_type_delivery active' : 'sb_type_delivery'},
					html:'Доставка',
					events: {click: BX.proxy(this.selectTypeDelivery, this)}
				...
				var itemNode = BX.create('DIV', {
					props: {className: 'sb_select_type_delivery'},
					children: [typeSamovivoz,typeDelivery],
				});

				BX.cleanNode(deliveryTitle);
				deliveryTypeContainer.appendChild(
						BX.create('h2', {
							props: {className: 'bx-soa-section-title col-sm-9'},
							html:'<span class="bx-soa-section-title-count"></span>Способ доставки',
						})
				);
				deliveryTypeContainer.appendChild(itemNode);
				deliveryTitle.appendChild(deliveryTypeContainer);
				...
				if (this.sbSelectedIdDelivery != this.sbSelectedIdDeliveryOld){

					this.selectDeliveryId(this.sbSelectedIdDelivery);
				}
		...
		selectTypeDelivery: function(event)
		{
			if (!this.orderBlockNode)
				return;

			var target = event.target || event.srcElement,
				sb_type = target.getAttribute('type');

			this.deliveryPickupDefaultType = sb_type;
			this.sbSelectedIdDelivery = null;
			this.sendRequest();
		},
		...
		editDeliveryItems: function(deliveryNode)
		{
			...
			var deliveryItemsContainer = BX.create('DIV', {props: {className: 'col-sm-12 bx-soa-pp-item-container'}}),
				deliveryItemNode, k, isFlagShow;


			for (k = 0; k < this.deliveryPagination.currentPage.length; k++)
			{
				if(this.deliveryPickupDefaultType == 'pickup'){
					isFlagShow = this.getActiveDeleivery(this.deliveryPagination.currentPage[k]);
				}else{
					isFlagShow = !this.getActiveDeleivery(this.deliveryPagination.currentPage[k]);
				}

				if(isFlagShow){
					if(this.sbSelectedIdDelivery == null){
						this.sbSelectedIdDelivery = this.deliveryPagination.currentPage[k].ID;
					}
					deliveryItemNode = this.createDeliveryItem(this.deliveryPagination.currentPage[k]);
					deliveryItemsContainer.appendChild(deliveryItemNode);
				}
		...
		editDeliveryInfo: function(deliveryNode)
		{
			...
			var deliveryNode = BX.findChildren(this.deliveryBlockNode, {className:'bx-soa-pp-company', attribute: {'data-id': currentDelivery.ID}}, true);
		...
		createDeliveryItem: function(item)
		{
			var checked = item.CHECKED == 'Y',
				...
				logotype, label, title, itemNode,
				costNode, descNode, periodNode, calculateNode, injectNode,
			 actionNode,
				 logoNode;

			logoNode = BX.create('DIV', {props: {className: 'sb-soa-pp-company-image'}});
			...
			if (item.PRICE >= 0 || typeof item.DELIVERY_DISCOUNT_PRICE !== 'undefined')
			{
				...
				costNode = BX.create('DIV', {
					...
					html: '<div class="bx-soa-pp-list-termin" lang="ru">' + this.params.MESS_PRICE + ':' + '</div>' + '<div class="bx-soa-pp-list-description" lang="ru">' + (item.PRICE > 0 ? (typeof item.DELIVERY_DISCOUNT_PRICE !== 'undefined'
						? item.DELIVERY_DISCOUNT_PRICE_FORMATED
						: item.PRICE_FORMATED) : this.params.MESS_PRICE_FREE) + '</div>'
				});
			}
			else if (deliveryCached && (deliveryCached.PRICE >= 0 || typeof deliveryCached.DELIVERY_DISCOUNT_PRICE !== 'undefined'))
			{
				...
				costNode = BX.create('DIV', {
					props: {className: 'bx-soa-pp-delivery-cost'},
					html: '<div class="bx-soa-pp-list-termin" lang="ru">' + this.params.MESS_PRICE + ':' + '</div>' + '<div class="bx-soa-pp-list-description" lang="ru">' + (deliveryCached.PRICE > 0 ? (typeof deliveryCached.DELIVERY_DISCOUNT_PRICE !== 'undefined'
						? deliveryCached.DELIVERY_DISCOUNT_PRICE_FORMATED
						: deliveryCached.PRICE_FORMATED) : this.params.MESS_PRICE_FREE) + '</div>'
				});
			}
			...
			if(item.ID == 73 && item.CHECKED=='Y'){
				actionNode = BX.create('DIV', {
					props: {id: 'sdek-vibor'},
					html: ''
				});
			}
			...
			itemNode = BX.create('DIV', {
				props: {className: 'sb_block-width50 bx-soa-pp-company bx-soa-pp-company-item' + (bHasPrice ? ' bx-soa-pp-company--hasprice' : '') + (bHasPeriod ? ' bx-soa-pp-company--hasperiod' : '') + (bHasCalculate ? ' bx-soa-pp-company--hascalculate' : '') + ' col-lg-4 col-sm-4 col-xs-6'},
				children: [logoNode, label, title, costNode, periodNode, calculateNode, injectNode, descNode,actionNode],
				events: {click: BX.proxy(this.selectDelivery, this)}
			});

			itemNode.setAttribute('data-id', item.ID);
			itemNode.setAttribute('id', 'sb_delivery_' + item.ID);
		...
		selectDeliveryId: function(SB_ID)
		{
			var target = BX('sb_delivery_' +SB_ID),
				actionCompany = BX.findParent(target, {className: 'bx-soa-pp-company-inject'}),
				actionSection =  BX.hasClass(target, 'bx-soa-pp-company-item') ? target : BX.findParent(target, {className: 'bx-soa-pp-company-item'}),
				selectedSection = this.deliveryBlockNode.querySelector('.bx-soa-pp-company-item.bx-selected'),
				actionInput, selectedInput;

			if(!actionCompany && !selectedSection){
				if (BX.hasClass(actionSection, 'bx-selected')){
					if (target.nodeName == 'INPUT' || target.nodeName == 'A' || target.nodeName == 'LABEL'){
						return;
					}
		...
		isValidForm: function()
		{
			...
			if (regionErrors.length)
			{
				...
				this.regionBlockNode.querySelector('.alert.alert-danger').style.display = 'none';
				BX.cleanNode(this.regionBlockNode.querySelector('.alert.alert-danger'));
				BX.removeClass(this.regionBlockNode, 'bx-step-error');
			}
		...
		getActiveDeleivery(item) {
			var res = false;
			for(var i = 0; i < this.deliveryPickupId.length; ++i){
				if(this.deliveryPickupId[i] == item.ID){
					return true;
				}
			}
			return res;
