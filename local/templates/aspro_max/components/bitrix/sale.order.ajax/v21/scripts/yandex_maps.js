...
(function () {
  ...
  BX.Sale.OrderAjaxComponent.Maps = {
    ...
    showNearestPickups: function (successCb, failCb) {
      ...
      var provider =
        this.pickUpOptions.secureGeoLocation && BX.browser.IsChrome() && !this.context.isHttps ? "yandex" : "auto";
    ...
    buildBalloons: function (activeStores) {
      ...
      for (var i = 0; i < activeStores.length; i++) {
        ...
        var geoObj = new ymaps.Placemark(
          [activeStores[i].GPS_N, activeStores[i].GPS_S],
          {
            hintContent:
              BX.util.htmlspecialchars(activeStores[i].TITLE) +
              "<br />" +
              BX.util.htmlspecialchars(activeStores[i].ADDRESS),
            ...
            balloonContentLayout: ymaps.templateLayoutFactory.createClass(
                ...
                '<br /><a class="btn btn-sm btn-default" data-store="{{ properties.id }}">{{ properties.text }}</a>',
              {
                build: function () {
                  ...
                  if (button) BX.bind(button, "click", this.selectStoreByClick);
                },
                clear: function () {
                  var button = document.querySelector("a[data-store]");
                  if (button) BX.unbind(button, "click", this.selectStoreByClick);
    ...
    selectBalloon: function (storeItemId) {
      if (this.pickUpMap && this.pickUpMap.geoObjects) {
        this.pickUpMap.geoObjects.each(
          BX.delegate(function (placeMark) {
    ...
    initializePropsMap: function (propsMapData) {
      ...
      this.propsMap.behaviors.disable(["scrollZoom", "ruler", "searchControl"]);

      this.propsMap.events.add(
        "click",
    ...
    getDistance: function (geoLocation, storeId) {
      if (!geoLocation || !storeId) return false;

      var storeGeoQuery = this.storeQueryResult[storeId];
      var distance = ymaps.coordSystem.geo.getDistance(
        geoLocation.geometry.getCoordinates(),
        storeGeoQuery.geometry.getCoordinates()
      );
