        ...
        <div class="region_wrapper">
            <div class="io_wrapper">
                ...
                <div class="js_city_chooser  animate-load  io_wrapper" data-event="jqm" data-name="city_chooser_small"
                    ...
                    <span><?= $arResult['CURRENT_REGION']['NAME']; ?></span><span
                            class="arrow"><?= CMax::showIconSvg("down",
                            SITE_TEMPLATE_PATH . "/images/svg/trianglearrow_down.svg"); ?></span>
                </div>
            </div>
            <? if ($arResult['SHOW_REGION_CONFIRM']): ?>
                <div class="confirm_region">
                    <span class="close colored_theme_hover_text "
                          data-id="<?= $arResult['CURRENT_REGION']['ID']; ?>"><?= CMax::showIconSvg('',
                            SITE_TEMPLATE_PATH . '/images/svg/Close.svg', '', 'light-ignore') ?></span>
                    <?
                    ...
                    <div class="buttons">
                        <span class="btn btn-default aprove" data-id="<?= $arResult['REAL_REGION']['ID']; ?>"
                              <?= $href; ?>><?= Loc::getMessage('CITY_YES'); ?></span>
    ...
    <div class="popup_regions <?= ($onlySearchRow ? 'only_search' : '') ?>">
        <div class="h-search autocomplete-block" id="title-search-city">
            <div class="wrapper">
                ...
                <div class="search_btn"><?= CMax::showIconSvg("search2",
                        SITE_TEMPLATE_PATH . "/images/svg/Search.svg"); ?></div>
            </div>
                ...
                <div class="favorits">
                    ...
                    <div class="cities">
                        <? foreach ($arResult['FAVORITS'] as $arItem): ?>
                            <div class="item">
                                <a href="<?= $arItem['URL']; ?>" data-id="<?= $arItem['ID']; ?>"
                                   class="name"><?= $arItem['NAME']; ?></a>
                            </div>
                        <? endforeach; ?>
                    </div>
                </div>
                            ...
                            <? foreach ($arResult['REGIONS'] as $key => $arItem): ?>
                                ...
                                <? if ($arItem['IBLOCK_SECTION_ID'] == $arSection['ID']): ?>
                                    ...
                                    <div class="item <?= ($bCurrent ? 'current' : ''); ?>"
                                         data-id="<?= ((isset($arItem['IBLOCK_SECTION_ID']) && $arItem['IBLOCK_SECTION_ID']) ? $arItem['IBLOCK_SECTION_ID'] : 0); ?>">
                                        <? if ($bCurrent): ?>
                                            <a href="<?= $arItem['URL']; ?>" data-id="<?= $arItem['ID']; ?>"><span
                                                        class="name"><?= $arItem['NAME']; ?></span></a>
                                        <? else: ?>
                                            <a href="https://<?= $arItem['PROPERTY_MAIN_DOMAIN_VALUE']; ?>" data-id="<?= $arItem['ID']; ?>"
                                               class="name dark_link"><?= $arItem['NAME']; ?></a>
                                        <? endif; ?>
                                    </div>
                                <? endif; ?>
                            <? endforeach; ?>
