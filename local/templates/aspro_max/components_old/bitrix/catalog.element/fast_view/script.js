	...
	if (-1 < index)
	{
		...
		if(!!this.offers[index].URL)
		{
			...
			if(arUrl.length > 1)
			{
				...
				if(arUrl2.length > 1)
				{
					$('.fast_view_frame').find('.form_head h2 a').attr('href', $('.fast_view_frame').find('.form_head h2 a').attr('href').replace(arUrl2[1], arUrl[1]));
					$('.fast_view_frame').find('.btn-wrapper a').attr('href', $('.fast_view_frame').find('.btn-wrapper a').attr('href').replace(arUrl2[1], arUrl[1]));
				}
				else
				{
		...
		if(bSideChar)
			$('.properties__container_js').empty();
		...
		if (this.config.showSkuProps)
		{
			...
			if (!this.offers[index].DISPLAY_PROPERTIES || this.offers[index].DISPLAY_PROPERTIES.length === 0)
			{
				if(display_type == "TABLE")
					...
					$(this.obProduct).find('.properties__container_js').html(html);
			}
			else
			{
				for(var i in this.offers[index].DISPLAY_PROPERTIES)
				{
					...
					else
					{
						...
						html = '<div class="properties__item properties__item--compact font_xs">'+
...
window.JCCatalogElementFast.prototype.SetSliderPict = function(obj, slider, config)
{
	...
	if(obj.POPUP_VIDEO)
	{
		var popup_video = '<div class="video-block popup_video '+(slider.length > 4 ? 'fromtop' : '')+' sm"><a class="various video_link image dark_link" href="'+obj.POPUP_VIDEO+'" title="'+BX.message('POPUP_VIDEO')+'"><span class="play text-upper font_xs">'+BX.message('POPUP_VIDEO')+'</span></a></div>';
