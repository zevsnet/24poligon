...
if (!empty($arResult['ITEMS'])){
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		...
		$arItem['GALLERY'] = \SB\Site\SB_CMax::getSliderForItemExt($arItem, $arParams['ADD_PICT_PROP'], 'Y' == $arParams['ADD_DETAIL_TO_SLIDER']);
			...
			if ('TYPE_1' == $arParams['TYPE_SKU'] && $arParams['DISPLAY_TYPE'] !='table')
			{
				...
				foreach ($arItem['OFFERS'] as $keyOffer => $arOffer)
				{
					...
					if ($arParams['SHOW_GALLERY'] == 'Y') {
						...
						$arItem['OFFERS'][$keyOffer]['GALLERY'] = \SB\Site\SB_CMax::getSliderForItemExt($arOffer, $arParams['OFFER_ADD_PICT_PROP'], true);
						if ($arItem['GALLERY']) {
				...
				if(empty($arItem['OFFERS_PROP']))
				{
					...
					if( $bNeedFindPicture ){
						foreach ($arItem['OFFERS'] as $keyOffer => $arOffer)
						{
							...
							$arFirstSkuPicture = \SB\Site\SB_CMax::getSliderForItemExt($arOffer, '', true);
