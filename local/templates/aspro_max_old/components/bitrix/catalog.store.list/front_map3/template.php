	...
	<div class="wrapper_block with_title title_right">
		...
		<div class="block_container bordered <?=($nCountItems == 1 ? 'one' : '');?>">
			<div class="block_container_inner">
				...
				<?endif;?>
				<div class="items scrollblock bordered" <?=($nCountItems == 1 ? "style='display:none;'" : "")?>>
					<div class="items-inner">
						<?foreach($arResult['MAP_ITEMS'] as $arItem):?>
							<div class="item" data-coordinates="<?=$arItem['GPS_N'].','.$arItem['GPS_S'];?>" data-id="<?=$arItem['ID']?>">
								<?=CMax::showIconSvg("addr muted ncolor", SITE_TEMPLATE_PATH."/images/svg/address.svg");?>
				...
				<div class="detail_items scrollblock" <?=($nCountItems == 1 ? "style='display:block;'" : "")?>>
					<?foreach($arResult['MAP_ITEMS'] as $arItem):?>
						<div class="item" <?=($nCountItems == 1 ? "style='display:block;'" : "")?> data-coordinates="<?=$arItem['GPS_N'].','.$arItem['GPS_S'];?>" data-id="<?=$arItem['ID']?>">
							<div class="big_info">
								<?=CMax::prepareItemMapHtml($arItem, "Y", $arParams, "Y");?>
							</div>
