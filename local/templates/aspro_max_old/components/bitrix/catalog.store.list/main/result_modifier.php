<?if($arResult["STORES"]):?>
	...
	if($arStoresIDs){
		...
		$dbRes = CCatalogStore::GetList(array('ID' => 'ASC'), array_merge($arFilter, $arrFilter), false, false, array("ID", "SORT", "TITLE", "IMAGE_ID", "ADDRESS", "EMAIL", "DESCRIPTION", "UF_METRO", "UF_PHONES"));
		while($arStore = $dbRes->GetNext()){
			if($arStoresByID[$arStore['ID']])
			{
				$arStoresByID[$arStore['ID']]["SORT"] = $arStore["SORT"];
				$arStoresByID[$arStore['ID']]["IMAGE"] = CFile::ResizeImageGet($arStore["IMAGE_ID"], array('width' => 450, 'height' => 300), BX_RESIZE_IMAGE_EXACT);
				...
				$arStoresByID[$arStore['ID']]["PHONE"] = array_merge(
					explode(",", $arStoresByID[$arStore['ID']]["PHONE"]), 
					(array)unserialize((string)$arStore['~UF_PHONES'])
				);
			}
		}
		foreach($arResult["STORES"] as $key => $arStore)
