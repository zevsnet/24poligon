 ...
 */
$tabIndex = 1;
$iMessageCount = $arResult['NAV_RESULT']->NavRecordCount;
?><? if ($arParams['SHOW_MINIMIZED'] == "Y")
{
	if(!$iMessageCount):?>
		<div class="empty-message clearfix">
			...
			<div class="text_before"><?=GetMessage("ANONS_TEXT_FORUM");?></div>
		</div>
	<?else:?>
		<div class="reviews-collapse reviews-minimized">
			<span class="reviews-collapse-link btn btn-default" id="sw<?=$arParams["FORM_ID"]?>"><i></i><span><?=$arParams['MINIMIZED_EXPAND_TEXT']?></span></span>
		</div>
		<?$this->SetViewTarget('PRODUCT_REVIWS_COUNT_INFO');?>
			(<?=$iMessageCount?>)
		<?$this->EndViewTarget();?>
	<?endif;
}
...
if (!empty($arResult["ERROR_MESSAGE"])):
	...
	?>
	<div class="reviews-note-box reviews-note-error alert alert-danger">
...
<script type="text/javascript">
	...
	<div style="position:relative; display: block; width:100%;">
			...
			<div class="reviews-reply-fields">
				<div class="reviews-reply-field-user">
					<div class="reviews-reply-field reviews-reply-field-author"><label for="REVIEW_AUTHOR<?=$arParams["form_index"]?>"><?=GetMessage("OPINIONS_NAME")?><?
						...
						<span><input class="form-control" name="REVIEW_AUTHOR" id="REVIEW_AUTHOR<?=$arParams["form_index"]?>" size="30" type="text" value="<?=$arResult["REVIEW_AUTHOR"]?>" tabindex="<?=$tabIndex++;?>" /></span></div>
					<?
					if ($arResult["FORUM"]["ASK_GUEST_EMAIL"]=="Y"):
						...
						<div class="reviews-reply-field reviews-reply-field-email"><label for="REVIEW_EMAIL<?=$arParams["form_index"]?>"><?=GetMessage("OPINIONS_EMAIL")?></label>
							<span><input class="form-control" type="text" name="REVIEW_EMAIL" id="REVIEW_EMAIL<?=$arParams["form_index"]?>" size="30" value="<?=$arResult["REVIEW_EMAIL"]?>" tabindex="<?=$tabIndex++;?>" /></span></div>
					<?
					endif;
					?>
			...
			<div class="row captcha-row form reviews-reply-field reviews-reply-field-captcha">
				<div class="col-md-6 col-sm-6 col-xs-6">
					<div class="form-group animated-labels reviews-reply-field-captcha-label">
						<label for="captcha_word"><?=GetMessage("F_CAPTCHA_PROMT")?> <span class="required-star">*</span></label>
						<div class="input">
							<input type="hidden" name="captcha_code" class="captcha_sid" value="<?=$arResult["CAPTCHA_CODE"]?>">
				...
				<div class="col-md-6 col-sm-6 col-xs-6">
					<div class="form-group">
						<div class="captcha-img">
							<div class="reviews-reply-field-captcha-image"><img src="/bitrix/tools/captcha.php?captcha_code=<?=$arResult["CAPTCHA_CODE"]?>" class="captcha_img" width="180" height="40" /></div>
		...
		<div class="reviews-reply-buttons">
			<button name="send_button" type="submit" class="btn btn-default" tabindex="<?=$tabIndex++;?>" <?
			?>onclick="this.form.preview_comment.value = 'N';" ><?=GetMessage("OPINIONS_SEND")?></button>
			<button name="view_button" type="submit" class="btn btn-default white" tabindex="<?=$tabIndex++;?>" <?
			?>onclick="this.form.preview_comment.value = 'VIEW';" ><?=GetMessage("OPINIONS_PREVIEW")?></button>
		</div>
