...
<?if($arResult):?>
	<?if (!function_exists('showSubItemss')) {
		function showSubItemss($arParams = [
			...
			$arParams['ITEM']["CHILD"] = $arParams['ITEM']["CHILD"] ?? [];
			?>
			...
			<?if($arParams['ITEM']["CHILD"] && $arParams['SHOW_CHILDS']):?>
				...
				<ul class="dropdown-menu toggle_menu">
					<?foreach($arParams['ITEM']["CHILD"] as $key => $arSubSubItem):?>
						<?
						$arSubSubItem["CHILD"] = $arSubSubItem["CHILD"] ?? [];
		...
		<table>
			<tr>
				<?foreach($arResult as $arItem):?>
					...
					$arItem["BANNERS"] = $arItem["BANNERS"] ?? false;
					$arItem["TEXT"] = $arItem["TEXT"] ?? false;
									...
									<div class="customScrollbar <?=($bWideMenu ? 'scrollblock scrollblock--thick' : '');?>">
										...
										<ul class="menu-wrapper <?='menu-type-'.$MENU_TYPE?>" >
											<?foreach($arItem["CHILD"] as $arSubItem):?>
												<?$arSubItem["CHILD"] = $arSubItem["CHILD"] ?? [];?>
													...
													<li class="<?=($arSubItem["CHILD"] && $bShowChilds ? "dropdown-submenu" : "")?> <?=$bIcon ? 'icon' : ''?> <?=($arSubItem["SELECTED"] ? "active" : "")?> <?=($bHasPicture ? "has_img" : "")?> parent-items">
														<?if($arItem['bManyItemsMenu']):?>
															<div class="flexbox flex-reverse">
																<?$bRightBanners = (isset($arSubItem["PARAMS"]["BANNERS"]) && $arSubItem["PARAMS"]["BANNERS"] && $arTheme['SHOW_RIGHT_SIDE']['DEPENDENT_PARAMS']['RIGHT_CONTENT']['VALUE'] == 'BANNER')?>
																<?$bRightBrands = (isset($arSubItem["PARAMS"]["BRANDS"]) && $arSubItem["PARAMS"]["BRANDS"] && isset($arSubItem["BRANDS"]) && $arSubItem["BRANDS"] && $arTheme['SHOW_RIGHT_SIDE']['DEPENDENT_PARAMS']['RIGHT_CONTENT']['VALUE'] == 'BRANDS');?>
																...
																<?if($bRightSide):?>
																	<div class="right-side">
																		<div class="right-content">
																			<?if($bRightBanners):?>
																				<?
																				if(isset($GLOBALS['arRegionLink']) && $GLOBALS['arRegionLink']) {
																...
																<div class="subitems-wrapper">
																	<ul class="menu-wrapper" >
																		<?foreach($arSubItem["CHILD"] as $arSubItem2):?>
																			<?
																			$arSubItem2["CHILD"] = $arSubItem2["CHILD"] ?? [];
