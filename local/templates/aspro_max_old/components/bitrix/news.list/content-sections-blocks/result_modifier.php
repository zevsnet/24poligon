...
$iSectionMaxServicesCount = isset($arParams['MAX_SERVICES_COUNT']) && intval($arParams['MAX_SERVICES_COUNT']) > 0 
	? intval($arParams['MAX_SERVICES_COUNT'])
	: false;
...
if($arResult['SECTIONS'])
{
	...
	if($arParams['SHOW_CHILD_SECTIONS'] == 'Y')
	{
		...
		foreach($arSections as $key => &$arSection){
			$iMaxChilds = $iSectionMaxServicesCount;
			$iChildsCount = isset($arSection['CHILD']) ? count($arSection['CHILD']) : 0;

			if( $iChildsCount > 0 && $iMaxChilds !== false ){
				array_splice($arSection['CHILD'], $iMaxChilds);
				$iMaxChilds -= $iChildsCount;
			}

			if( $iMaxChilds === false || $iMaxChilds > 0 ){
				$arNavStartParams = $iMaxChilds > 0 ? ['nPageSize' => $iMaxChilds] : false;
				$arItems = CMaxCache::CIBlockElement_GetList(array('SORT' => 'ASC', 'ID' => 'DESC', 'CACHE' => array('TAG' => CMaxCache::GetIBlockCacheTag($arParams['IBLOCK_ID']), "CACHE_GROUP" => array($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()))), array_merge($arSectionsFilter, array('SECTION_ID' => $arSection['ID']), $arItemsRegionFilter), false, $arNavStartParams, array('ID', 'NAME', 'IBLOCK_ID', 'DETAIL_PAGE_URL'));
				
				if($arItems)
					$arSections[$key]['CHILD'] = !$arSections[$key]['CHILD'] ? $arItems : array_merge($arSections[$key]['CHILD'], $arItems);
			}
		}
		unset($arSection);
