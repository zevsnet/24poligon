				...
				<div class="group-content">
					<?foreach($arResult['SECTIONS'] as $SID => $arSection):?>
						<div class="tab-pane">
							...
							<div class="items row flexbox <?=$bCarousel ? "swipeignore mobile-overflow mobile-margin-16 mobile-compact" : ""?>">
								<?foreach($arSection['ITEMS'] as $i => $arItem):?>
									...
									?>
									<div class="col-lg-<?=$size;?> col-md-<?=$size_md;?> col-sm-4 col-xs-6 item-wrap <?=$bCarousel ? "item-width-261" : ""?>">
																	...
																	<?foreach($arItem['MIDDLE_PROPS'] as $key => $arProp):?>
																		<div class="prop">
																			...
																			<?if($arProp['CODE'] == 'EMAIL'):?>
																				<!-- noindex --><a class="dark-color" href="mailto:<?=$arProp['VALUE'];?>" target="_blank" rel="nofollow"><?=$arProp['VALUE'];?></a>
