...
<?if($arResult['SECTIONS']):?>
	...
	<?//if(!$isAjax):?>
		...
		<div class="items row <?=($arParams['ACCORDION']=='Y' ? 'accordion-type-1 ' : '');?> <?=$bCarousel ? "swipeignore mobile-overflow mobile-margin-16 mobile-compact" : ""?>">
			<?// show section items?>
	<?//endif;?>
			<?
			...
			<?foreach($arSection['ITEMS'] as $i => $arItem):?>	    
				...
				<div class="col-md-12 <?=$bCarousel ? "item-width-261" : ""?>">
					<div class="item_wrap box-shadow rounded3 <?=($arParams['BORDERED']=='Y' ? 'bordered-block ' : '')?> <?=$bCarousel ? "item" : ""?> <?=($arParams['ACCORDION']=='Y' ? 'item-accordion-wrapper ' : ' colored_theme_hover_bg-block ');?>" >
