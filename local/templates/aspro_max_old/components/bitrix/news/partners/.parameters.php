...
if(\Bitrix\Main\Loader::includeModule('iblock'))
{
	...
	$arProperty_S = $arProperty_XL = $arProperty_N = $arProperty_ALL = array();
...
if ($viewTemplate === 'FROM_MODULE') {
	if (isset($_SESSION) 
		...
		&& isset($_SESSION['THEME'][$siteID]['PARTNERS_PAGE'])
	) {
		$viewTemplate = $_SESSION['THEME'][$siteID]['PARTNERS_PAGE'];
	} else {
		$viewTemplate = \Bitrix\Main\Config\Option::get(CMax::moduleID, 'PARTNERS_PAGE', '', $siteID);
	}
}
if(strpos($viewTemplate, 'with_group') !== false){
	...
	$arTemplateParameters["SIDE_LEFT_BLOCK"] = Array(
		"PARENT" => "BASE",
	...
	$arTemplateParameters["TYPE_LEFT_BLOCK"] = Array(
		"PARENT" => "BASE",
