...
<?endif;?>
	...
	<div class="opener">
		...
		<div title="<?=$arCounters['DELAY']['TITLE']?>" data-type="DelDelCanBuy" class="colored_theme_hover_text wish_count small clicked<?=(!$arCounters['DELAY']['COUNT'] ? ' empty' : '')?>">
			<a href="<?=$arCounters['DELAY']['HREF']?>" class="delay"></a>
	...
	?>
		...
		<?}?>
		<?if ($normalCount):?>
			<script src="<?=(((COption::GetOptionString('main', 'use_minified_assets', 'N', SITE_ID) === 'Y') && file_exists($_SERVER['DOCUMENT_ROOT'].SITE_TEMPLATE_PATH.'/js/buy_services.min.js')) ? SITE_TEMPLATE_PATH.'/js/buy_services.min.js' : SITE_TEMPLATE_PATH.'/js/buy_services.js')?>" type="text/javascript"></script>
			<link rel="stylesheet" href="<?=SITE_TEMPLATE_PATH;?>/css/bonus-system.min.css"/>
			<link rel="stylesheet" href="<?=SITE_TEMPLATE_PATH;?>/css/buy_services.min.css"/>
		<?endif;?>
