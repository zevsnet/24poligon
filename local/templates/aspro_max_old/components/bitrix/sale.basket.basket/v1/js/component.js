	...
	BX.Sale.BasketComponent = {
		...
		resizeServicesBasket: function()
		{
			...
			if(window.matchMedia('(min-width: 768px)').matches){
				var widthColumnPrice = $('tr:not(.hidden-basket-services) .basket-items-list-item-price:not(.basket-items-list-item-price-for-one):eq(0)').width();
				var paddingPriceCol = 9;
				...
				if(servicesPriceBlock.length){
					...
					$('.services_in_basket_page .services-item__info').css('max-width', 'calc(100% - ' + widthColumnPrice-paddingPriceCol + ')');
				}				
				...
				if (this.shownItems.length && sortIndex >= 0)
				{
					...
					else if (sortIndex > BX.util.array_search(this.shownItems[this.shownItems.length - 1], this.sortedItems))
					{
						...
						if(BX.type.isDomNode(BX(this.ids.itemServices + this.shownItems[this.shownItems.length - 1]))){
							BX(this.ids.itemServices + this.shownItems[this.shownItems.length - 1]).insertAdjacentHTML('afterend', basketItemHtml);
						} else {
