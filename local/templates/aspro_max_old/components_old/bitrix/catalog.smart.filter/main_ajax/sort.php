...
foreach($arAvailableSort as $key => $val){
	...
	$arResult['ITEMS']['ASPRO_FILTER_SORT']['VALUES'][] = array(
		'CONTROL_HTML' => '<span class="sort_btn js-load-link '.($bSelected ? 'current' : '').' '.$newSortOrder.' '.$key.'" data-url="'.$url.'" onclick="AjaxClickLink(this)" data-click_block=".wrapper_inner .js-load-link.'.$newSortOrder.'.'.$key.'">'.GetMessage('SECT_SORT_'.$key).' ('.GetMessage($newSortOrder).')'.'</span>',
