...
foreach ($arResult["MESSAGES"] as $res):
	...
	?><table class="reviews-post-table <?=($iCount == 1 ? "reviews-post-first " : "")?><?
		...
		?>" bx-author-id="<?=$res["AUTHOR_ID"]?>" bx-author-name="<?=$res["AUTHOR_NAME"]?>" id="message<?=$res["ID"]?>" itemprop="review" itemscope itemtype="http://schema.org/Review">
		<thead><tr><td>
		<?
...
<div>
		...
		if (intVal($res["AUTHOR_ID"]) > 0 && !empty($res["AUTHOR_URL"])):
			?><a href="<?=$res["AUTHOR_URL"]?>" class="dark-link"><span itemprop="author"><?=$res["AUTHOR_NAME"]?></span></a><?
		else:
			?><span itemprop="author" class="darken"><?=$res["AUTHOR_NAME"]?></span><?
		endif;
		?>
		...
		<span class='message-post-date muted'><?=$res["POST_DATE"]?></span>
</div>
	...
	<tbody>
	<tr><td>
		<div class="reviews-text muted777 font_sm" id="message_text_<?=$res["ID"]?>" itemprop="reviewBody"><?=$res["POST_MESSAGE_TEXT"]?></div>
<?
...
?>
<div class="reviews-note-box reviews-note-note alert alert-success">
	<a name="reviewnote"></a>
