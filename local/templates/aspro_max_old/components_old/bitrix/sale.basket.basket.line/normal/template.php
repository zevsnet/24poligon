...
$bShowActual = ($arParams['SHOW_ACTUAL'] == 'Y');
...
if($bShowActual)
{
	...
	echo \Bitrix\Main\Web\Json::encode(array(
		'BASKET_SUMM' => ($arResult['NUM_PRODUCTS'] ? GetMessage("BASKET_COUNT", array("#PRICE#" => CMax::clearFormatPrice($arResult['TOTAL_PRICE']))) : GetMessage("EMPTY_BLOCK")),
		'DELAY_SUMM' => ($sDelayPrice ? GetMessage("BASKET_DELAY_COUNT", array("#PRICE#" => CMax::clearFormatPrice($sDelayPrice))) : GetMessage("EMPTY_BLOCK")),
		)
	);
}
else
{
	...
	usort($arBasketIDs, 'CMax::cmpByID');?>
	...
	<div class="basket_normal cart <?=(!$count || $isBasketPage || $_POST["ACTION"]=='top' ? ' empty_cart ' : '')?> <?=(!$count && $isBasketPage ? 'ecart' : '');?> <?=($isBasketPage || $_POST["ACTION"]=='top' ? 'bcart' : '');?>">
		<!--noindex-->
			<div class="wraps_icon_block delay <?=($delayCount ? 'fill' : '' );?>">
				<a href="<?=$arParams["PATH_TO_BASKET"];?>#delayed" class="link" <?=($delayCount ? '' : 'style="display: none;"' );?> title="<?=GetMessage("BASKET_DELAY_LIST");?>"></a>
				<?if($delayCount){?>
					<div class="count">
						<span>
							<div class="items">
									...
									<tbody>
										...
										foreach($arBasketIDs as $arItem){
											if(($arItem["CAN_BUY"] == "Y") && ($arItem["DELAY"] == "N")){
												...
												$item=CMaxCache::CIBLockElement_GetList(array('CACHE' => array("MULTI"=>"N", "TAG" => CMaxCache::GetIBlockCacheTag($arItem["IBLOCK_ID"]))), array("ACTIVE"=>"Y", "ACTIVE_DATE" => "Y", "ID" => $arItem["PRODUCT_ID"]), false, false, array("ID", "DETAIL_PAGE_URL", "IBLOCK_ID", "PROPERTY_CML2_LINK", "PREVIEW_PICTURE", "DETAIL_PICTURE"));
												if($item["PROPERTY_CML2_LINK_VALUE"]){
													$itemLink=CMaxCache::CIBLockElement_GetList(array('CACHE' => array("MULTI"=>"N", "TAG" => CMaxCache::GetIBlockCacheTag($item["IBLOCK_ID"]))), array("ACTIVE"=>"Y", "ACTIVE_DATE" => "Y", "ID" => $item["PROPERTY_CML2_LINK_VALUE"]), false, false, array("ID", "PREVIEW_PICTURE", "IBLOCK_ID", "DETAIL_PICTURE"));
													if($itemLink){
														$item["ID"] = $itemLink["ID"];
