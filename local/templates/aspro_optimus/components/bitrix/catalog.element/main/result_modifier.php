...
if ($productSlider) {
    foreach ($productSlider as $i => $arImage) {
        ...
        $productSlider[$i] = array_merge(
            $arImage, array(
                "BIG" => \SB\Site\General::getarWaterMark(340, $arImage["ID"], true),
                "SMALL" => \SB\Site\General::getarWaterMark(340, $arImage["ID"]),
                "THUMB" => \SB\Site\General::getarWaterMark(50, $arImage["ID"]),
            )
        );
    }
}
        ...
        if ($arOffer['MORE_PHOTO']) {
            foreach ($arOffer['MORE_PHOTO'] as $i => $arImage) {
                if ($arImage["ID"]) {
                    ...
                    $arOffer['MORE_PHOTO'][$i]["SMALL"] = \SB\Site\General::getarWaterMark(340, $arImage["ID"]);
                    $arOffer['MORE_PHOTO'][$i]["THUMB"] = \SB\Site\General::getarWaterMark(52, $arImage["ID"]);
                }
            }
        }
    ...
    if ($obCache->InitCache(3600000, serialize($tizerCacheID), "/hlblock/tizers")) {
        ...
        if ($arData = $rsData->fetch()) {
            ...
            while ($arEnum = $rsPropEnums->fetch()) {
                if ($arEnum["UF_FILE"]) {
                    $arEnum['PREVIEW_PICTURE'] = \SB\Site\General::getarWaterMark(45, $arEnum['UF_FILE']);
                }
                $arItems[] = $arEnum;
            }
        }
...
/*
if ($arResult['PROPERTIES']['STARAYA_TSENA']) {
    if ($arResult['PROPERTIES']['STARAYA_TSENA']['VALUE'] !== '') {
        $arResult['MIN_PRICE']["DISCOUNT_DIFF"] = $minPrice["PRINT_VALUE"] = CurrencyFormat($arResult['PROPERTIES']['STARAYA_TSENA']['VALUE'], 'RUB');
        $arResult['MIN_PRICE']['PRINT_VALUE_VAT'] = $arResult['MIN_PRICE']["DISCOUNT_DIFF"];

        $arResult['MIN_PRICE']['VALUE_VAT'] = $arResult['PROPERTIES']['STARAYA_TSENA']['VALUE'];
        $arResult['MIN_PRICE']['VALUE'] = $arResult['PROPERTIES']['STARAYA_TSENA']['VALUE'];
    }
}

foreach ($arResult['OFFERS'] as $key => $arOffer) {
    $arResult['OFFERS'][$key]['MIN_PRICE']['VALUE_VAT'] = $arResult['PROPERTIES']['STARAYA_TSENA']['VALUE'];
    $arResult['OFFERS'][$key]['MIN_PRICE']['VALUE'] = $arResult['PROPERTIES']['STARAYA_TSENA']['VALUE'];

    $arResult['OFFERS'][$key]['MIN_PRICE']['PRINT_VALUE_VAT'] = $arResult['PROPERTIES']['STARAYA_TSENA']['VALUE'];
    $arResult['OFFERS'][$key]['MIN_PRICE']['PRINT_VALUE'] = $arResult['PROPERTIES']['STARAYA_TSENA']['VALUE'];
}*/
