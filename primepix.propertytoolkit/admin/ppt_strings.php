...
if (!empty($arProps)) {
	...
	foreach($arProps as $key => $arProp) {
		...
		// get values list
		$dbSubRes  = $dbOp->getValuesList($arProp['ID']);
		$valCount  = $dbSubRes->SelectedRowsCount();

		while($arSubRes = $dbSubRes->Fetch()) {
			if (preg_match('/^[\.,\d]+$/i' , trim($arSubRes['VALUE']))) {
				$posValCount++;
			}	
		}

		if ($posValCount == $valCount) {
		...
		$propTableRows[] = CPptUtils::getView(
			$PATH_TO_VIEWS . 'table_string_row',
			array(
				'arProp'             => $arProp,
				'dataEmpty'          => $dataEmpty,
				'dataSame'           => $dataSame,
...
$propTable = CPptUtils::getView(
	$PATH_TO_VIEWS . 'table_string_header',
			...
			<div class="footer-panel -footer-panel">
				<div class="controls-wrap">
					...
					<?if ($convertPropsCount > 1):?>
						<button class="-convert-all -view-help adm-btn" data-possible="<?=$posPropCount?>" data-action="CONVERT_TO_NUMBER">
