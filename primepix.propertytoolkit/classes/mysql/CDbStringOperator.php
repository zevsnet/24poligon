...
/**
 * Class CDbStringOperator
 * Class to work with props type string
 */
class CDbStringOperator extends CDbBaseOperator
{

	protected static $type = 'S';
	...
	/**
	 * Function will convert array of props to number type, round values and replace to valid num
	...
	public function convertProp2Number($propsIds) {
		...
		foreach ($propsIds as $propId) {
			...
			while ($arRes = $dbRes->Fetch()) {
				...
				$valueNum = round($value, 4);

				$strSql = sprintf('UPDATE `%s` SET VALUE = "%s", VALUE_NUM = "%s" WHERE IBLOCK_PROPERTY_ID = %d AND ID = %d',
				self::TABLE_IB_EL_PROP,
				$value,
				$valueNum,
				(int)$arRes["ID"],
	...
	/**
	 * Function will convert array of props to list type, insert values into enum
	 * @param  array $propsIds
	 */
